...
final class DiscountValidateHandler extends BaseHandler
{
    public function handle(): void
    {
        ...
        $code = RedFoxInput::string($this->data, 'code');
        if ($code === '') {
            RedFoxResponse::badRequest('code is required');
        }

        $ctxIn = RedFoxInput::string($this->data, 'context');
        ...
        $codeProduct = RedFoxInput::string($this->data, 'product_code');
        $codePanel   = RedFoxInput::string($this->data, 'code_panel');
        ...
        if (empty($result['ok'])) {
            RedFoxResponse::fail(422, (string)($result['reason'] ?? '❌ کد تخفیف نامعتبر است.'));
        }

        $row = $result['row'];
        $base = RedFoxInput::int($this->data, 'base_price', 0);
