...
final class CryptoInvoiceInitHandler extends BaseHandler
{
    ...
    public function handle(): void
    {
        ...
        $currency     = strtoupper(RedFoxInput::string($this->data, 'currency_code'));
        $purchaseUser = RedFoxInput::nullableString($this->data, 'purchase_username');

        if ($amount <= 0) {
            RedFoxResponse::badRequest('amount must be > 0');
        }
        if ($currency === '') {
            RedFoxResponse::badRequest('currency_code is required');
        }
        $supported = crypto_supported_currencies();
        if (!isset($supported[$currency])) {
            RedFoxResponse::fail(422, '❌ ارز انتخابی پشتیبانی نمی‌شود');
        }
        ...
        if ($purchaseUser !== null && $purchaseUser !== '') {
            $unpaidExists = (int) RedFoxDb::fetchScalar(
        ...
        try {
            ...
            RedFoxLogger::exception($e, 'crypto_create_invoice threw', [
                'user' => $this->user['id'], 'currency' => $currency, 'amount' => $amount,
            ]);
            RedFoxResponse::serverError('خطا در ساخت فاکتور');
        }
