...
final class BrandHandler extends BaseHandler
{
    ...
    public function handle(): void
    {
        ...
        RedFoxResponse::badRequest('Brand mode invalid');
    }
    ...
    private function handleSave(): void
    {
        ...
        if (!$this->userIsAdmin()) {
            RedFoxResponse::fail(403, 'admin only');
        }

        $name = RedFoxInput::string($this->data, 'name');
        $mark = RedFoxInput::string($this->data, 'mark');
        $accent = strtolower(trim(RedFoxInput::string($this->data, 'accent')));
        ...
        if ($accent !== '') {
            if (!preg_match('/^#[0-9a-fA-F]{6}$/', $accent)) {
                RedFoxResponse::badRequest('accent must be a hex color like #7c5cff');
            }
            $this->upsertSetting('brand_accent', $accent);
        }

        RedFoxResponse::ok(self::buildBrandPayload() + ['message' => 'برند با موفقیت ذخیره شد']);
    }
    ...
    private function handleUpload(): void
    {
        ...
        $clear = RedFoxInput::string($_POST, 'clear');
        if ($clear === '1') {
            $this->deleteOldLogo();
            $this->upsertSetting('brand_logo', '');
            RedFoxResponse::ok(self::buildBrandPayload() + ['message' => 'لوگو حذف شد']);
        }

        if (!isset($_FILES['logo']) || !is_array($_FILES['logo'])) {
            RedFoxResponse::badRequest('logo file is required');
        }
        $f = $_FILES['logo'];
        if ((int)($f['error'] ?? 99) !== UPLOAD_ERR_OK) {
            RedFoxResponse::badRequest('upload error: ' . ($f['error'] ?? 'unknown'));
        }
        $uploadSize = (int)($f['size'] ?? 0);
        if ($uploadSize < 1 || $uploadSize > 4 * 1024 * 1024) {
            RedFoxResponse::badRequest('logo too large (max 4 MB)');
        }
        $tmp = (string)($f['tmp_name'] ?? '');
        if ($tmp === '' || !is_uploaded_file($tmp) || !is_readable($tmp)) {
            RedFoxResponse::badRequest('logo not accessible on server');
        }


        $info = @getimagesize($tmp);
        if (!is_array($info)) {
            RedFoxResponse::badRequest('invalid image format');
        }
        $width = (int)($info[0] ?? 0);
        $height = (int)($info[1] ?? 0);
        if ($width < 1 || $height < 1 || $width > 2048 || $height > 2048 || ($width * $height) > 4194304) {
            RedFoxResponse::badRequest('image dimensions are too large');
        }
        $mime = (string)($info['mime'] ?? '');
        $allowed = ['image/png', 'image/jpeg', 'image/webp', 'image/gif'];
        if (!in_array($mime, $allowed, true)) {
            RedFoxResponse::badRequest('image type not supported');
        }
        ...
        if (!is_dir($brandDir) || !is_writable($brandDir)) {
            RedFoxResponse::fail(503, 'branding folder is not writable on server');
        }
        ...
        if (!$resized) {
            RedFoxResponse::fail(500, 'image resize failed (GD missing or write error)');
        }
        ...
        RedFoxResponse::ok(self::buildBrandPayload() + ['message' => 'لوگو ذخیره شد']);
    }
    ...
    private function upsertSetting(string $name, string $value): void
    {
        try {
            ...
            RedFoxLogger::exception($e, 'brand upsertSetting failed', ['setting' => $name]);
            RedFoxResponse::fail(500, 'cannot save brand setting');
        }
    }
