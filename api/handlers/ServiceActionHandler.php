...
final class ServiceActionHandler extends BaseHandler
{
    ...
    public function handle(): void
    {
        ...
        if ($action === '' || !isset(self::ACTION_LABELS[$action])) {
            ...
            RedFoxResponse::badRequest('Invalid action');
        }
        if ($username === '') {
        ...
        $payload = RedFoxInput::array($this->data, 'payload');
        [$err, $extraBlock] = $this->validatePayload($action, $payload);
        if ($err !== null) {
            RedFoxResponse::fail(422, '❌ ' . $err);
        }
        ...
        try {
            ...
            RedFoxLogger::exception($e, (string)('admin table fetch failed (service_action)'));
        }
        if (empty($adminIds)) {
        ...
        if (!is_string($keyboard)) {
            RedFoxResponse::fail(500, 'ساخت درخواست مدیریت سرویس ناموفق بود.');
        }
        ...
        if ($sent === 0) {
            RedFoxResponse::fail(502, '❌ ارسال درخواست به ادمین ناموفق بود. لطفاً دوباره تلاش کنید.');
        }

        RedFoxLogger::debug('Service action request notified', [
            'user'    => $this->user['id'],
            'action'  => $action,
    ...
    private function performChangeLink(array $invoice): void
    {
        ...
        if ($panelName === '' || $svcUsername === '') {
            RedFoxResponse::fail(422, '❌ اطلاعات سرویس ناقص است.');
        }

        $panel = select('marzban_panel', '*', 'name_panel', $panelName, 'select');
        if (!is_array($panel) || empty($panel)) {
            RedFoxResponse::fail(404, '❌ پنل سرویس پیدا نشد.');
        }
        ...
        try {
            ...
            if (!method_exists($managePanel, 'Revoke_sub')) {
                RedFoxResponse::fail(503, '❌ امکان تغییر لینک روی این سرور فعال نیست.');
            }
            $result = $managePanel->Revoke_sub($panelName, $svcUsername);
        } catch (Throwable $e) {
            RedFoxLogger::exception($e, 'changelink Revoke_sub threw', [
                'user'    => $this->user['id'],
                'invoice' => $invoice['id_invoice'] ?? null,
            ]);
            RedFoxResponse::fail(502, '❌ خطایی در تغییر لینک رخ داد. لطفاً دوباره تلاش کنید.');
        }

        if (!is_array($result) || (isset($result['status']) && $result['status'] === 'Unsuccessful')) {
            $safeCode = redfox_remote_error_summary($result);
            RedFoxLogger::warn('changelink Revoke_sub failed', [
                'user'       => $this->user['id'],
                'invoice'    => $invoice['id_invoice'] ?? null,
                'error_code' => $safeCode,
                'error_ref'  => redfox_remote_error_fingerprint($result),
            ]);
            RedFoxResponse::fail(502, '❌ تغییر لینک ناموفق بود. کد: ' . $safeCode);
        }
...
▫️زمان تغییر لینک : {$timejalali}";
        ...
        RedFoxLogger::debug('Service changelink completed', [
            'user'    => $this->user['id'],
            'invoice' => $invoice['id_invoice'] ?? null,
        ]);
    ...
    private function sendAdminMessage(string $apiKey, string $chatId, string $text, string $keyboardJson): bool
    {
        try {
            ...
            RedFoxLogger::exception($e, 'Administrator notification failed');
            return false;
        }
    }
}
