...
final class ServiceRenewConfirmHandler extends BaseHandler
{
    public function handle(): void
    {
        ...
        if (!in_array((string)$invoice['Status'], ['active', 'end_of_time', 'end_of_volume', 'sendedwarn', 'send_on_hold'], true)) {
            RedFoxResponse::fail(409, '❌ تمدید با خطا مواجه گردید مراحل تمدید را مجددا انجام دهید.');
        }
        ...
        if (($panel['status_extend'] ?? '') === 'off_extend') {
            RedFoxResponse::fail(409, '❌ امکان تمدید در این پنل وجود ندارد');
        }

        $agent = (string)($this->user['agent'] ?? 'f');


        $custom = RedFoxInput::array($this->data, 'custom');
        if (!empty($custom)) {
            $product = $this->buildCustomProduct($panel, $custom, $agent);
        } else {
            $code = RedFoxInput::string($this->data, 'product_code');
            if ($code === '') {
                RedFoxResponse::badRequest('product_code or custom is required');
            }
            $row = RedFoxDb::fetchOne(
                "SELECT * FROM product
            ...
            if (!is_array($row)) {
                RedFoxResponse::fail(404, '❌ خطایی رخ داده است مراحل تمدید را از اول انجام دهید.');
            }
        ...
        if ($maxBuyAgent !== 0 && $agent === 'n2') {
            if (($balance - $finalPrice) < (-1 * $maxBuyAgent)) {
                RedFoxResponse::fail(403, '❌ مبلغ مجاز خرید شما به اتمام رسیده است.');
            }
        }
        ...
        if ($shortfall > 0 && $agent !== 'n2') {
            ...
            if (!$directBuyEnabled) {
                RedFoxResponse::fail(402, '❌ موجودی کیف پول شما کافی نیست. لطفاً ابتدا کیف پول را شارژ کنید.');
            }
            ...
            try {
                ...
                RedFoxResponse::fail(500, '❌ خطا در ثبت درخواست تمدید. لطفاً دوباره تلاش کنید.');
            }
        ...
        if ($finalPrice > 0) {
            ...
            if (empty($charge['ok'])) {
                RedFoxLogger::warn('Atomic balance charge failed at renew', [
        ...
        try {
            ...
            RedFoxLogger::exception($e, 'ManagePanel->extend threw at renew', [
                'user_id'  => $this->user['id'],
            ...
            RedFoxResponse::fail(502, '❌ خطایی در تمدید سرویس رخ داده با پشتیبانی در ارتباط باشید');
        }


        if (!is_array($extend) || ($extend['status'] ?? null) === false) {
            $reason = redfox_remote_error_summary($extend);
            RedFoxLogger::error('ManagePanel->extend failed', [
                'user_id'  => $this->user['id'],
        ...
        try {
            ...
            $stmt->execute([
                ...
                ':o' => json_encode(['status' => true], JSON_UNESCAPED_SLASHES),
            ]);
        } catch (Throwable $e) {
            RedFoxLogger::exception($e, (string)('service_other insert failed (extend)'));
        }
        ...
        RedFoxLogger::debug('Inline renewal completed', [
            'user_id'   => $this->user['id'],
            'username'  => $invoice['username'],
    ...
    private function buildCustomProduct(array $panel, array $custom, string $agent): array
    {
        ...
        if ($volume > $maxVol || $volume < $minVol) {
            RedFoxResponse::badRequest("❌ حجم نامعتبر است (بین {$minVol} و {$maxVol} گیگابایت)");
        }
        if ($time > $maxTime || $time < $minTime) {
            RedFoxResponse::badRequest("❌ زمان نامعتبر است (بین {$minTime} و {$maxTime} روز)");
        }
    ...
    private function reportSuccess(string $text): void
    {
        ...
        try {
            ...
            RedFoxLogger::exception($e, (string)('renew success report failed'));
        }
    }
}
