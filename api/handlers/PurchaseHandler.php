...
final class PurchaseHandler extends BaseHandler
{
    public function handle(): void
    {
        ...
        if (empty($panel)) {
            RedFoxResponse::fail(404, 'پنل انتخابی موجود نیست.');
        }
        if (($panel['status'] ?? '') === 'disable') {
            RedFoxResponse::fail(409, 'پنل انتخابی درحال حاضر فعال نیست');
        }


        $customService = RedFoxInput::array($this->data, 'custom_service');
        if (empty($customService)) {
            $serviceId = RedFoxInput::string($this->data, 'service_id');
            if ($serviceId === '') {
                RedFoxResponse::badRequest('service_id is required');
            }
        ...
        if (empty($product)) {
            RedFoxResponse::fail(404, 'محصول انتخابی پیدا نشد');
        }

        if (!$this->productIsAllowedForAgent((array)$product, $this->user['agent'])) {
            RedFoxResponse::fail(403, 'این محصول برای نوع کاربری شما فعال نیست');
        }
        ...
        $discountCode = RedFoxInput::string($this->data, 'discount_code');
        if ($discountCode !== '') {
        ...
        $customUsername = RedFoxInput::nullableString($this->data, 'custom_username');
        $methodUsername = (string)($panel['MethodUsername'] ?? '');
        if ($methodUsername === 'نام کاربری دلخواه') {
            $trimmedCustom = trim((string)$customUsername);
            if ($trimmedCustom === '') {
                RedFoxResponse::fail(422, 'برای این پنل، انتخاب نام کاربری دلخواه ضروری است.');
            }
            if (!preg_match('/^[A-Za-z0-9_.-]{3,40}$/', $trimmedCustom)) {
                RedFoxResponse::fail(422, 'نام کاربری معتبر نیست. فقط حروف انگلیسی، عدد، _ . - (۳ تا ۴۰ کاراکتر).');
            }
        ...
        $existsLocal = RedFoxDb::fetchScalar(
        ...
        if ($existsLocal || (is_array($remoteCheck) && isset($remoteCheck['username']))) {
            RedFoxResponse::fail(409, 'نام کاربری وجود دارد مراحل را از اول طی کنید');
        }


        $customNote = RedFoxInput::nullableString($this->data, 'custom_note');
        ...
        if ($shortfall > 0.0) {
            ...
            if (!$directBuyEnabled) {
                RedFoxResponse::fail(402, 'موجودی کمتر از قیمت محصول است');
            }
            ...
            try {
                ...
                RedFoxLogger::exception($e, 'Unpaid invoice insert failed', ['user_id' => $this->user['id']]);
                RedFoxResponse::serverError('خطا در ذخیره فاکتور');
            }
            ...
            RedFoxLogger::debug('Direct-buy initiated', [
        ...
        try {
            ...
            RedFoxLogger::exception($e, 'Invoice insert failed', ['user_id' => $this->user['id']]);
            RedFoxResponse::serverError('خطا در ذخیره فاکتور');
        }
        ...
        if ($priceToCharge > 0.0) {
            ...
            if (empty($charge['ok'])) {
                RedFoxLogger::warn('Atomic balance charge failed at purchase', [
                    'user_id'  => $this->user['id'],
                    'order_id' => $orderId,
                    'price'    => $priceToCharge,
                    'reason'   => $charge['reason'] ?? 'unknown',
                ]);
                try { RedFoxDb::execute('DELETE FROM invoice WHERE id_invoice = :o AND id_user = :u', [':o' => $orderId, ':u' => $this->user['id']]); } catch (Throwable $_) {}
                RedFoxResponse::fail(402, 'موجودی کافی نیست (تلاش هم‌زمان شناسایی شد). یک بار دیگر تلاش کنید.');
            }
        ...
        if (empty($remote['username'])) {
            $reason = redfox_remote_error_summary($remote);
            RedFoxLogger::error('createUser failed', [
                'user_id' => $this->user['id'],
            ...
            RedFoxResponse::serverError('خطایی در ساخت اشتراک رخ داده است با پشتیبانی در ارتباط باشید');
        }
        ...
        $invoiceCount = (int) RedFoxDb::fetchScalar(
        ...
        RedFoxLogger::debug('Purchase completed', [
            'user_id'  => $this->user['id'],
            'order_id' => $orderId,
            'panel'    => $panel['name_panel'],
            'product'  => $product['code_product'] ?? null,
    ...
    private function sendInfoCardNotification(
        ...
        try {
            ...
            RedFoxLogger::exception($e, (string)('createServiceInfoCard threw'));
            return false;
        }
        ...
        try {
            ...
            @unlink($outPath);
            RedFoxLogger::exception($e, (string)('sendphoto (info card) failed'));
            return false;
        }
    }
    ...
    private function buildCustomProduct(array $panel, array $customService): array
    {
        ...
        if ($volume > $maxVolume || $volume < $minVolume) {
            RedFoxResponse::badRequest('حجم نامعتبر است خرید را از اول انجام دهید');
        }
        if ($time > $maxTime || $time < $minTime) {
            RedFoxResponse::badRequest('زمان نامعتبر است خرید را از اول انجام دهید');
        }
    ...
    {
        $balanceAfter = (float) RedFoxDb::fetchScalar(
