...
final class PaymentInitHandler extends BaseHandler
{
    ...
    public function handle(): void
    {
        ...
        $method = RedFoxInput::string($this->data, 'method');
        $amount = RedFoxInput::int($this->data, 'amount', 0);

        if ($method === '') {
            RedFoxResponse::badRequest('method is required');
        }
        if ($amount <= 0) {
            RedFoxResponse::badRequest('amount must be > 0');
        }
        ...
        if ($method === 'carttocart' || $method === 'carttocart_pv') {
            ...
            $pendingFresh = (int) RedFoxDb::fetchScalar(
        ...
        $renewUsername = RedFoxInput::nullableString($this->data, 'renew_username');
        $purchaseUsername = RedFoxInput::nullableString($this->data, 'purchase_username');
        if ($renewUsername !== null && $renewUsername !== '') {

            $serviceExists = (int) RedFoxDb::fetchScalar(
                "SELECT COUNT(*) FROM invoice WHERE username = :u AND id_user = :uid",
                [':u' => $renewUsername, ':uid' => $this->user['id']]
            );
            if ($serviceExists === 0) {
                RedFoxResponse::fail(404, '❌ سرویسی برای تمدید با این نام کاربری پیدا نشد.');
            }
            ...
            if (!in_array($tow, $allowedTow, true) || $one === '' || strpos($one, '%') === false) {
                RedFoxResponse::fail(409, '❌ مراحل تمدید کامل نشده است. لطفاً تمدید را از ابتدا انجام دهید.');
            }

        } elseif ($purchaseUsername !== null && $purchaseUsername !== '') {


            $unpaidInvoiceExists = (int) RedFoxDb::fetchScalar(
                "SELECT COUNT(*) FROM invoice
                  WHERE username = :u AND id_user = :uid AND Status = 'unpaid'",
                [':u' => $purchaseUsername, ':uid' => $this->user['id']]
            );
            if ($unpaidInvoiceExists === 0) {
                RedFoxResponse::fail(404, '❌ فاکتور خرید ناتمامی برای این نام کاربری پیدا نشد.');
            }
            ...
            $chargeCode = RedFoxInput::string($this->data, 'discount_code');
            if ($chargeCode !== '') {
                $dv = MiniDiscount::validateSell($chargeCode, 'charge', '', '', $this->user);
                if (empty($dv['ok'])) {
                    RedFoxResponse::fail(422, (string)($dv['reason'] ?? '❌ کد تخفیف نامعتبر است.'));
                }
                $gatewayAmount = (int) round(MiniDiscount::applyToPrice($dv['row'], (float)$amount));
                if ($gatewayAmount <= 0) {
                    RedFoxResponse::fail(422, '❌ این کد برای شارژ قابل استفاده نیست (مبلغ پرداختی صفر می‌شود).');
                }
        ...
        RedFoxResponse::badRequest('Unknown payment method: ' . $method);
    }
    ...
    private function purgeStaleCarttocart(int $userId): void
    {
        try {
            ...
            RedFoxLogger::exception($e, (string)('purgeStaleCarttocart fetch failed'));
            return;
        }
        ...
        try {
            ...
            $stmt->execute($params);
            RedFoxLogger::debug('Purged stale carttocart Unpaid rows', [
                'user_id' => $userId,
                'count'   => count($stale),
            ]);
        } catch (Throwable $e) {
            RedFoxLogger::exception($e, (string)('purgeStaleCarttocart delete failed'));
        }
    }
    ...
    private function handleCardToCard(int $amount, callable $get): void
    {
        ...
        try {
            ...
            RedFoxLogger::exception($e, (string)('card_number fetch failed'));
        }
        if (!is_array($card) || empty($card['cardnumber']) || empty($card['namecard'])) {
            RedFoxResponse::fail(503, '❌ کارت بانکی فعالی برای کارت‌به‌کارت تنظیم نشده است.');
        }
    ...
    private function handleGateway(string $method, int $amount, callable $createFn, callable $extractUrl): void
    {
        ...
        if (!is_callable($createFn)) {
            RedFoxResponse::fail(503, '❌ تابع گیت‌وی در سرور موجود نیست: ' . $method);
        }

        try {
            $pay = $createFn($amount, $orderId);
        } catch (Throwable $e) {
            RedFoxLogger::exception($e, (string)('Gateway threw: ' . $method));
            RedFoxResponse::fail(502, '❌ خطا در ارتباط با درگاه ' . $method);
        }

        $url = $extractUrl($pay);
        if (!$url) {
            RedFoxLogger::userFacing('Gateway did not return a URL: ' . $method, ['raw' => $pay]);
            RedFoxResponse::fail(502, '❌ ساخت لینک پرداخت ناموفق بود. لطفاً دوباره تلاش کنید.');
        }
    ...
    private function handleIranpay1(int $amount): void
    {
        if (!function_exists('createInvoiceiranpay1')) {
            RedFoxResponse::fail(503, '❌ تابع درگاه ارزی ریالی روی این سرور موجود نیست.');
        }
        ...
        if ($min > 0 && $amount < $min) {
            RedFoxResponse::fail(422, '❌ حداقل مبلغ پرداخت ' . number_format($min) . ' تومان است.');
        }
        if ($max > 0 && $amount > $max) {
            RedFoxResponse::fail(422, '❌ حداکثر مبلغ پرداخت ' . number_format($max) . ' تومان است.');
        }
        ...
        try {
            ...
            RedFoxLogger::exception($e, (string)('createInvoiceiranpay1() threw'));
            RedFoxResponse::fail(502, '❌ خطا در ارتباط با درگاه پرداخت.');
        }
        ...
        if ($paymentUrl === '' || $authority === '') {
            ...
            RedFoxLogger::userFacing('createInvoiceiranpay1() returned bad response', ['raw' => $errMsg]);
            RedFoxResponse::fail(502, '❌ ساخت لینک پرداخت ناموفق بود.');
        }
    ...
    private function handlePlisio(int $amount): void
    {
        if (!function_exists('plisio')) {
            RedFoxResponse::fail(503, '❌ تابع Plisio روی این سرور موجود نیست.');
        }
        ...
        if (!function_exists('requireTronRates')) {
            RedFoxResponse::fail(503, '❌ ماژول نرخ ارز روی سرور بارگذاری نشده.');
        }
        $rates = requireTronRates(['TRX', 'USD']);
        if (!is_array($rates) || !isset($rates['TRX'], $rates['USD'])) {
            RedFoxResponse::fail(503, '❌ دریافت نرخ ارز ناموفق بود. لطفاً چند دقیقه دیگر تلاش کنید.');
        }
        $trx = (float)$rates['TRX'];
        $usd = (float)$rates['USD'];
        if ($trx <= 0 || $usd <= 0) {
            RedFoxResponse::fail(503, '❌ نرخ ارز نامعتبر است. لطفاً مدتی دیگر تلاش کنید.');
        }
        $usdPrice = round($amount / $usd, 2);
        if ($usdPrice <= 1) {
            RedFoxResponse::fail(422, '❌ مبلغ پرداخت بسیار کم است (کمتر از 1 دلار).');
        }
        ...
        try {
            ...
            RedFoxLogger::exception($e, (string)('plisio() threw'));
            RedFoxResponse::fail(502, '❌ خطا در ارتباط با Plisio. لطفاً دوباره تلاش کنید.');
        }
        if (!is_array($pay) || empty($pay['txn_id']) || empty($pay['invoice_url'])) {
            RedFoxLogger::log(RedFoxLogger::WARNING, 'plisio() returned invalid response shape', ['is_array' => is_array($pay)]);
            RedFoxResponse::fail(502, '❌ ساخت لینک پرداخت Plisio ناموفق بود.');
        }
    ...
    private function handleNowPayment(int $amount): void
    {
        if (!function_exists('nowPayments')) {
            RedFoxResponse::fail(503, '❌ تابع NowPayments روی این سرور موجود نیست.');
        }
        ...
        $usd = (float)$rates['USD'];
        if ($usd <= 0) {
            RedFoxResponse::fail(503, '❌ نرخ ارز نامعتبر است.');
        }
        $usdPrice = round($amount / $usd, 2);
        if ($usdPrice <= 0) {
            RedFoxResponse::fail(422, '❌ مبلغ پرداخت بسیار کم است.');
        }
        ...
        try {
            ...
            RedFoxLogger::exception($e, (string)('nowPayments() threw'));
            RedFoxResponse::fail(502, '❌ خطا در ارتباط با NowPayments.');
        }
        if (!is_array($pay) || empty($pay['id']) || empty($pay['invoice_url'])) {
            $errMsg = is_array($pay) ? json_encode($pay) : 'unknown';
            RedFoxLogger::userFacing('nowPayments() returned bad response', ['raw' => $errMsg]);
            RedFoxResponse::fail(502, '❌ ساخت لینک پرداخت NowPayments ناموفق بود.');
        }
    ...
    private function botDeepLink(string $startParam): string
    {
        ...
        if ($bot === '') {
            RedFoxResponse::fail(503, '❌ نام کاربری ربات روی سرور ثبت نشده است.');
        }
    ...
    private function insertPaymentReport(string $method, int $amount, string $orderId, ?string $extId = null): void
    {
        ...
        try {
            ...
            RedFoxLogger::exception($e, 'Payment report insert failed', ['has_external_id' => $extId !== null]);
            RedFoxResponse::fail(503, 'ثبت درخواست پرداخت ناموفق بود؛ لطفاً دوباره تلاش کنید.');
        }
    }
