...
final class CryptoCancelInvoiceHandler extends BaseHandler
{
    public function handle(): void
    {
        ...
        if (!in_array($method, $cancellable, true)) {
            RedFoxResponse::fail(422, 'این فاکتور قابل لغو از این طریق نیست');
        }
        ...
        if (!in_array($status, ['Unpaid', 'AwaitingHash'], true)) {
            RedFoxResponse::fail(409, 'این فاکتور در وضعیتی نیست که قابل لغو باشد (' . $status . ')');
        }
        ...
        try {
            ...
            RedFoxLogger::exception($e, 'Crypto cancel-invoice update failed', [
                'user'  => $this->user['id'],
                'order' => $orderId,
            ]);
            RedFoxResponse::serverError('خطا در لغو فاکتور');
        }

        RedFoxLogger::debug('Crypto invoice cancelled by user', [
            'user'  => $this->user['id'],
            'order' => $orderId,
        ]);
