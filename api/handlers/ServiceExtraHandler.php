...
final class ServiceExtraHandler extends BaseHandler
{
    ...
    {
        $mode = $this->mode ?? RedFoxInput::string($this->data, '_mode');
        if ($mode === 'quote') {
        ...
        RedFoxResponse::badRequest('Unknown mode for service_extra');
    }
    ...
    private function handleConfirm(): void
    {
        ...
        if (!ctype_digit((string)$amount)) {
            RedFoxResponse::badRequest('amount must be a positive integer');
        }
        if (count($bounds) === 2 && $bounds[0] > 0 && $amount < $bounds[0]) {
            RedFoxResponse::fail(422, "❌ مقدار باید حداقل {$bounds[0]} باشد");
        }
        if (count($bounds) === 2 && $bounds[1] > 0 && $amount > $bounds[1]) {
            RedFoxResponse::fail(422, "❌ مقدار باید حداکثر {$bounds[1]} باشد");
        }
        ...
        if ($finalPrice > 0) {
            ...
            if (empty($charge['ok'])) {
                RedFoxLogger::warn('Atomic balance charge failed at extra', [
        ...
        if (!is_array($result) || ($result['status'] ?? null) === false) {
            $reason = redfox_remote_error_summary($result);
            RedFoxLogger::error('ManagePanel extra_* failed', [
                'kind'     => $kind,
                'user_id'  => $this->user['id'],
        ...
        try {
            ...
            RedFoxLogger::exception($e, (string)('service_other insert failed (extra)'));
        }
        ...
        RedFoxLogger::debug('Inline extra purchase completed', [
            'kind'     => $kind,
            'user_id'  => $this->user['id'],
            'username' => $invoice['username'],
            'amount'   => $amount,
    ...
    private function resolveContext(string $method): array
    {
        ...
        $kind = strtolower(RedFoxInput::string($this->data, 'kind'));
        if (!in_array($kind, ['time', 'volume'], true)) {
            RedFoxResponse::badRequest("kind must be 'time' or 'volume'");
        }
        ...
        if ($method === 'POST'
            ...
            RedFoxResponse::fail(409, '❌ خرید با خطا مواجه گردید مراحل را مجدد انجام دهید.');
        }
        ...
        if (($panel['status_extend'] ?? '') === 'off_extend') {
            RedFoxResponse::fail(409, $kind === 'time'
        ...
        if ($kind === 'time' && ($invoice['name_product'] ?? '') === 'سرویس تست') {
            RedFoxResponse::fail(409, '❌ این قابلیت برای سرویس تست در دسترس نیست');
        }
        ...
        if ($pricePerUnit <= 0) {
            RedFoxResponse::fail(503, '❌ تعرفه خرید اضافه روی این پنل تنظیم نشده است.');
        }
