...
final class ServicesHandler extends BaseHandler
{
    public function handle(): void
    {
        ...
        $categoryId = RedFoxInput::nullableString($this->data, 'category_id');
        $timeRangeDay = RedFoxInput::nullableString($this->data, 'time_range_day');

        $categoryRow = null;
        if ($categoryId !== null && $categoryId !== '0') {
            $categoryRow = select('category', '*', 'id', $categoryId, 'select');
            if (!is_array($categoryRow) || !isset($categoryRow['remark'])) {
                RedFoxResponse::badRequest('category not found (invalid category_id)');
            }
        }
        ...
        $rows = RedFoxDb::fetchAll($sql, $params);
