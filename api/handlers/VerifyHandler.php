...
final class VerifyHandler
{
    public static function run(): void
    {
        ...
        if ($userData === null) {
            ...
            RedFoxLogger::warn('initData verification failed', [
                'exception' => $lastException ? get_class($lastException) : 'missing_init_data',
            ]);
            self::sendVerifyError($status, 'Telegram authentication is invalid or expired');
        }
        ...
        if (empty($userRecord)) {
            ...
            if (empty($userRecord) || !is_array($userRecord)) {
                RedFoxLogger::warn('Auto-registration failed for miniapp user', ['tg_id' => $userId]);
    ...
    public static function finalizeForUser(int $userId, array $userData, array $userRecord): void
    {
        ...
        try {
            ...
            $existingExpiry = (int)($userRecord['token_expires_at'] ?? 0);
            $token = ($existingToken !== '' && $existingExpiry > time() + 60)
                ? $existingToken
                : RedFoxAuth::issueToken($userId);
        } catch (Throwable $e) {
            RedFoxLogger::exception($e, 'Failed to issue token');
            self::sendVerifyError(500, 'Failed to generate session token');
        }

        RedFoxLogger::debug('User verified', ['user_id' => $userId]);
    ...
    private static function autoRegisterUser(int $userId, array $userData): ?array
    {
        ...
        if (!($pdo instanceof PDO)) {
            RedFoxLogger::critical('Cannot auto-register miniapp user: database connection unavailable');
            return null;
        }
        ...
        if (!is_array($setting)) {
            RedFoxLogger::critical('Cannot auto-register miniapp user: settings unavailable');
            return null;
        }
        ...
        try {
            ...
            RedFoxLogger::exception($e, 'Auto-registration insert failed');
            return null;
        }
        ...
        RedFoxLogger::debug('Auto-registered new miniapp user', ['tg_id' => $userId]);
        return $userRecord;
    }
