...
final class MiniDiscount
{
    ...
    public static function redeemGift(string $code, array $user): array
    {
        ...
        $row = RedFoxDb::fetchOne('SELECT * FROM Discount WHERE code = :c LIMIT 1', [':c' => $code]);
        if ($row === null) {
        ...
        $newBalance = RedFoxDb::fetchScalar('SELECT Balance FROM user WHERE id = :u', [':u' => $user['id']]);
    ...
    public static function validateSell(string $code, string $section, string $codeProduct, string $codePanel, array $user, bool $blockIfUserDiscount = true): array
    {
        ...
        $usedByUser = (int) RedFoxDb::fetchScalar(
    ...
    public static function markSellUsed(string $code, array $user): void
    {
        ...
        try {
            $pdo = RedFoxDb::pdo();
            $row = RedFoxDb::fetchOne('SELECT usedDiscount FROM DiscountSell WHERE codeDiscount = :c LIMIT 1', [':c' => $code]);
    ...
    public static function releaseLastUnpaidDiscount(string $userId, ?string $discountCode = null, ?int $referenceTime = null): bool
    {
        ...
        try {
            ...
            $dsRow = RedFoxDb::fetchOne('SELECT usedDiscount FROM DiscountSell WHERE codeDiscount = :c LIMIT 1', [':c' => $code]);
            if (is_array($dsRow)) {
