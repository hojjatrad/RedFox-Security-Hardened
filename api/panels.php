...
switch ($data['actions'] ?? '') {
    ...
    case 'panels':
        ...
        try {
            ...
            error_log("Database error in panel: " . redfox_exception_fingerprint($e));
    ...
    case 'panel_add':
        ...
        try {
            ...
            error_log("Error in panel_add: " . redfox_exception_fingerprint($e));
    ...
    case 'panel_edit':
        ...
        if (isset($data['name']) && $panel['name_panel'] != $data['name']) {
            ...
            $panel_check = select("marzban_panel", "*", "name_panel", $data['name'], "count");
            if ($panel_check != 0)
                sendJsonResponse(false, "panel name exits", [], 200);
        ...
        try {
            ...
            $stmt = $pdo->prepare("UPDATE marzban_panel SET {$setClause} WHERE id = :id");
            ...
            error_log("Error in panel_edit: " . redfox_exception_fingerprint($e));
            sendJsonResponse(false, "An error occurred while updating panel");
        }
        break;
    case 'panel_delete':
        validateMethod('POST', $method);
        ...
        try {
            ...
            error_log("Error in panel delete : " . redfox_exception_fingerprint($e));
    ...
    case 'set_inbounds':
        ...
        if ($panel['type'] == "marzban") {
            ...
            if (!empty($DataUserOut['error']))
                sendJsonResponse(false, redfox_remote_error_summary($DataUserOut), [], 200);
            ...
            if (!empty($user_data['error']))
                sendJsonResponse(false, redfox_remote_error_summary($user_data), [], 200);
