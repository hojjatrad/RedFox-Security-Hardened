...
if (class_exists('RedFoxInput')) {
    return;
}

final class RedFoxInput
{
    ...
    public static function payload(): array
    {
        ...
        if ($method === 'POST' || $method === 'PUT' || $method === 'PATCH' || $method === 'DELETE') {
            ...
            if ($contentType === 'application/json') {
                ...
                if (!is_string($raw) || trim($raw) === '' || strlen($raw) > 131072) {
        ...
        if (count($payload, COUNT_RECURSIVE) > 512) {
            throw new InvalidArgumentException('Payload is too complex');
        }
