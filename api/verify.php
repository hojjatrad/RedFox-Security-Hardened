...
if ($declaredLength < 1 || $declaredLength > 32768) {
...
try {
    ...
    $verifyRate = redfox_login_rate_check('miniapp-verify', redfox_client_ip(), 60, 300);
    if (empty($verifyRate['allowed'])) {
        if (!headers_sent()) header('Retry-After: ' . max(1, (int)($verifyRate['retry_after'] ?? 300)));
