...
function logApiRequest($headers, $data, $action)
{
    ...
    try {
        ...
        error_log("API logging error: " . redfox_exception_fingerprint($e));
    }
}


$headers = rx_integration_redacted_headers();
...
switch ($data['actions'] ?? '') {
    ...
    case 'users':
        ...
        try {
            ...
            error_log("Database error in users: " . redfox_exception_fingerprint($e));
    ...
    case 'user':
        ...
        try {
            ...
            error_log("Database error in user: " . redfox_exception_fingerprint($e));
            sendJsonResponse(false, "Database error occurred", [], 500);
        }
        break;
    ...
    case 'user_add':
        ...
        try {
            ...
            if (!$userInfo['ok']) {
                sendJsonResponse(false, redfox_remote_error_summary($userInfo));
            }
            ...
            error_log("Error in user_add: " . redfox_exception_fingerprint($e));
...
اکنون میتوانید از ربات استفاده کنید ✔️", null, 'HTML');
    ...
    case 'transfer_account':
        ...
        try {
            ...
            error_log('transfer_account failed and was rolled back: ' . redfox_exception_fingerprint($e));
    ...
    case 'active_bot_agent':
        ...
        $getInfoToken = telegram('getMe', [], (string)$data['token']);
        if (!is_array($getInfoToken) || empty($getInfoToken['ok']) || empty($getInfoToken['result']['username']))
        ...
        if (!preg_match('/^[A-Za-z0-9_]{1,64}$/', $remoteUsername)) {
            error_log('[api/users active_bot_agent] rejected unsafe bot username from getMe');
        ...
        $dirsource = "$destination/vpnbot/{$data['chat_id']}{$remoteUsername}";


        if (is_link($defaultSource) || !is_dir($defaultSource)) {
            error_log('[api/users active_bot_agent] default bot template unavailable');
        ...
        if ((is_dir($dirsource) || is_link($dirsource)) && !deleteDirectory($dirsource)) {
            error_log('[api/users active_bot_agent] failed to remove existing bot directory');
            sendJsonResponse(false, "Failed to clean previous bot directory", [], 500);
        }
        if (!copyDirectoryContents($defaultSource, $dirsource)) {
            error_log('[api/users active_bot_agent] failed to copy default bot files');
        ...
        if (!is_string($contentconfig)
            || substr_count($contentconfig, 'BotTokenNew') !== 1
            || substr_count($contentconfig, 'WebhookSecretNew') !== 1) {
            deleteDirectory($dirsource);
            error_log('[api/users active_bot_agent] invalid bot config template');
            sendJsonResponse(false, "Bot config template invalid", [], 500);
        }
        $new_code = str_replace(['BotTokenNew', 'WebhookSecretNew'], [(string)$data['token'], $webhookSecret], $contentconfig);
        $written = file_put_contents($configFile, $new_code, LOCK_EX);
        if ($written !== strlen($new_code) || !@chmod($configFile, 0600)) {
            deleteDirectory($dirsource);
            error_log('[api/users active_bot_agent] failed to write bot config');
            sendJsonResponse(false, "Failed to configure bot", [], 500);
        }
        $applicationBase = redfox_configured_base_url();
        if ($applicationBase === '') {
            deleteDirectory($dirsource);
            sendJsonResponse(false, 'Webhook base URL invalid', ['code' => 'TG-WH-URL'], 500);
        }
        $hookUrl = $applicationBase . "/vpnbot/{$data['chat_id']}{$remoteUsername}/index.php";
        $botTokenForWebhook = (string)$data['token'];
        $webhookResult = redfox_apply_telegram_webhook(
            static function (string $method, array $parameters) use ($botTokenForWebhook): array {
                $result = telegram($method, $parameters, $botTokenForWebhook);
        ...
        if (empty($webhookResult['ok'])) {
            deleteDirectory($dirsource);
            $safeCode = preg_match('/^[A-Z0-9_-]{1,64}$/', (string)($webhookResult['code'] ?? ''))
                ? (string)$webhookResult['code'] : 'TG-WH-UNKNOWN';
            error_log('[api/users active_bot_agent] webhook rejected code=' . $safeCode);
            sendJsonResponse(false, 'Webhook setup failed', ['code' => $safeCode], 502);
        }
        telegram('sendMessage', [
            'chat_id' => $data['chat_id'],
        ...
        $value = "{}";
        try {
            $stmt = $pdo->prepare("INSERT INTO botsaz (id_user,bot_token,admin_ids,username,time,setting,hide_panel,webhook_secret_token) VALUES (:id_user,:bot_token,:admin_ids,:username,:time,:setting,:hide_panel,:webhook_secret_token)");
            $stmt->bindValue(':id_user', $data['chat_id'], PDO::PARAM_STR);
            $stmt->bindValue(':bot_token', $data['token'], PDO::PARAM_STR);
            $stmt->bindValue(':admin_ids', $admin_ids, PDO::PARAM_STR);
            $stmt->bindValue(':username', $remoteUsername, PDO::PARAM_STR);
            $stmt->bindValue(':time', date('Y/m/d H:i:s'), PDO::PARAM_STR);
            $stmt->bindValue(':setting', $datasetting, PDO::PARAM_STR);
            $stmt->bindValue(':hide_panel', $value, PDO::PARAM_STR);
            $stmt->bindValue(':webhook_secret_token', $webhookSecret, PDO::PARAM_STR);
            $stmt->execute();
        } catch (Throwable $error) {
            telegram('deleteWebhook', [], (string)$data['token']);
            deleteDirectory($dirsource);
            error_log('[api/users active_bot_agent] database insert failed: ' . redfox_exception_fingerprint($error));
            sendJsonResponse(false, "Failed to save bot", [], 500);
        }
        sendJsonResponse(true, "Successful");
        break;
    case "remove_agent_bot":
        ...
        if (!isset($data['chat_id']) || empty($data['chat_id']))
            sendJsonResponse(false, "user-id empty", [], 200);
        $removeChatId = html_entity_decode((string)$data['chat_id'], ENT_QUOTES, 'UTF-8');
        if (!preg_match('/^-?\d{3,20}$/', $removeChatId)) {
            sendJsonResponse(false, "chat_id invalid", [], 400);
        }
        $data['chat_id'] = $removeChatId;
        ...
        try {
            ...
            error_log("Failed to delete botsaz row for user {$data['chat_id']}: " . redfox_exception_fingerprint($e));
            sendJsonResponse(false, "Failed to remove bot record", [], 500);
        }

        $destination = dirname(__DIR__);
        $storedUsername = (string)($contentbot['username'] ?? '');
        if (preg_match('/^[A-Za-z0-9_]{1,64}$/', $storedUsername)) {
            $dirsource = "$destination/vpnbot/{$data['chat_id']}{$storedUsername}";
            if ((is_dir($dirsource) || is_link($dirsource)) && !deleteDirectory($dirsource)) {
                error_log('[api/users remove_agent_bot] failed to remove bot directory');
            }
        } else {
            error_log('[api/users remove_agent_bot] skipped unsafe stored bot username');
        }
        $storedBotToken = (string)($contentbot['bot_token'] ?? '');
        if (preg_match('/^\d+:[A-Za-z0-9_\-]{20,}$/', $storedBotToken)) {
            telegram('deleteWebhook', [], $storedBotToken);
        }
        sendJsonResponse(true, "Successful");
        break;
