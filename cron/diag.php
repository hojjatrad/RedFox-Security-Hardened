...
// Diagnostics has an independent secret and never accepts credentials in a URL.
$provided = (string) ($_SERVER['HTTP_X_DIAG_SECRET'] ?? '');
$expected = (string) (rx_env('REDFOX_DIAG_SECRET') ?: '');
if ($expected === '' || $provided === '' || !hash_equals($expected, $provided)) {
    http_response_code(404);
    exit;
}

// The loopback probe is also authenticated; it cannot be used as an open ping.
if (isset($_GET['ping'])) {
...
function rx_diag_curl(string $url, string $secret, ?string $resolveTo = null): array
{
    ...
    return [
        ...
        'code' => $code,
        'err'  => $errno === 0 ? '' : 'curl_errno_' . $errno,
        'body' => ($body !== false && $errno === 0 && is_string($body)) ? substr($body, 0, 60) : '',
    ];
}
...
$domainhosts = redfox_normalize_domain((string)($GLOBALS['domainhosts'] ?? ''));
if ($domainhosts === '') {
    http_response_code(503);
    exit('REDFOX_DOMAIN is missing or invalid.');
}
$domainParts = parse_url('https://' . $domainhosts);
$hostName = is_array($domainParts) ? (string)($domainParts['host'] ?? '') : '';
if ($hostName === '') {
    http_response_code(503);
    exit('REDFOX_DOMAIN is invalid.');
}
$hostOnly = str_contains($hostName, ':') ? '[' . $hostName . ']' : $hostName;
if (isset($domainParts['port']) && (int)$domainParts['port'] !== 443) $hostOnly .= ':' . (int)$domainParts['port'];
$basePart = rtrim((string)($domainParts['path'] ?? ''), '/');
...
$testPublic = $hostOnly !== '' ? rx_diag_curl($pingUrl, $expected, null) : ['ok' => false, 'code' => 0, 'err' => 'domainhosts خالی است'];
$testLoop   = $hostOnly !== '' ? rx_diag_curl($pingUrl, $expected, '127.0.0.1') : ['ok' => false, 'code' => 0, 'err' => 'domainhosts خالی است'];
...
if ($pdo instanceof PDO) {
    try {
        ...
        error_log('[cron-diag] runtime-state lookup failed: ' . redfox_exception_fingerprint($e));
        $runtimeErr = 'runtime_state_unavailable';
    }
}
...
<div class="box">
    ...
    <p>۲) یا HTTP با Header اختصاصی (مقدار Secret را در URL قرار ندهید):</p>
    <p><code>curl --fail --silent --show-error -H "X-Cron-Secret: YOUR_CRON_SECRET" "https://<?php echo htmlspecialchars($hostOnly . $basePart, ENT_QUOTES); ?>/cron/cron.php" &gt;/dev/null 2&gt;&amp;1</code></p>
