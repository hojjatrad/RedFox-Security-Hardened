...
if (is_readable($functionBootstrap)) {
    try {
        ...
        if ($rxLogIt) {
            error_log('[cron.php] bootstrap failed: ' . redfox_exception_fingerprint($e));
...
$host = isset($domainhosts) && is_string($domainhosts) ? redfox_normalize_domain($domainhosts) : '';
if ($host === '') {
    @unlink($lockFile);
    echo "SKIP (REDFOX_DOMAIN missing or invalid)\n";
    exit;
}
$hostConfig = 'https://' . $host;
...
if ($hostOnly === '') {
    @unlink($lockFile);
    echo "SKIP (REDFOX_DOMAIN invalid)\n";
    exit;
}
$authority = str_contains($hostOnly, ':') ? '[' . $hostOnly . ']' : $hostOnly;
if (isset($parts['port']) && (int)$parts['port'] !== 443) $authority .= ':' . (int)$parts['port'];

$buildCronUrl = static function (string $script) use ($scheme, $authority, $basePath): string {
    $script = ltrim($script, '/');
    return $scheme . '://' . $authority . $basePath . '/cronbot/' . $script;
};
...
if (function_exists('rx_cron_telemetry_start')) rx_cron_telemetry_start($pdo,'orchestrator');
...
$rxCronSecret = (string)(rx_env('REDFOX_CRON_SECRET') ?: '');
$dispatchAsync = static function (array $urls, bool $useLoopback) use ($rxCronSecret): array {
    ...
    foreach ($urls as $url) {
        ...
        $rxHeaders = [
            ...
            'X-Cron-Source: cron-orchestrator', 'Connection: close',
        ];
        if ($rxCronSecret !== '') $rxHeaders[] = 'X-Cron-Secret: ' . $rxCronSecret;
        $curlOpts = [
