...
{
    $safeContext = preg_replace('/[^A-Za-z0-9_.-]/', '_', (string)$context) ?: 'wireguard';
    $safeStatus = is_numeric($status) ? (int)$status : 0;
    $bodyBytes = is_string($body) ? strlen($body) : 0;
    $flags = ($curlError ? ' transport_error' : '') . ($jsonError ? ' json_error' : '');
    error_log(sprintf('[wireguard] %s HTTP=%d bytes=%d%s', $safeContext, $safeStatus, $bodyBytes, $flags));
}
...
function decodeJsonResponse($response, string $context)
{
    ...
    if (json_last_error() !== JSON_ERROR_NONE) {
        ...
        return [
            ...
            'http_status' => is_numeric($httpStatus) ? (int)$httpStatus : 0,
        ];
    }
...
function extractAvailableIp(array $availableIpResponse)
{
    if (empty($availableIpResponse['data']) || !is_array($availableIpResponse['data'])) {
        error_log('[wireguard] getAvailableIPs returned no data array');
        return [
            'status' => false,
    ...
    error_log('[wireguard] getAvailableIPs returned no usable entries');
    return [
        'status' => false,
        'msg' => 'No available IPs',
    ];
...
function get_userwg($username,$namepanel){
    ...
    if (!is_array($response)) {
        error_log('[wireguard] configuration response was not an array');
        return [
            'status' => false,
    ...
    if (empty($response['data']) || !is_array($response['data'])) {
        error_log('[wireguard] configuration response missing data');
        return [
            'status' => false,
