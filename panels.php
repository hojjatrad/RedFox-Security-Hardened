...
if (!function_exists('redfox_invoice_subscription_url')) {
    function redfox_invoice_subscription_url(array &$invoice): string
    {
        global $pdo;
        $invoiceId = trim((string)($invoice['id_invoice'] ?? ''));
        if ($invoiceId === '' || strlen($invoiceId) > 191 || !($pdo instanceof PDO)) {
            throw new RuntimeException('Subscription invoice is invalid');
        }
        $token = strtolower(trim((string)($invoice['subscription_token'] ?? '')));
        if (!preg_match('/^[a-f0-9]{64}$/', $token)) {
            $candidate = bin2hex(random_bytes(32));
            try {
                $stmt = $pdo->prepare(
                    "UPDATE invoice SET subscription_token=:token
                      WHERE id_invoice=:invoice
                        AND (subscription_token IS NULL OR subscription_token='')"
                );
                $stmt->execute([':token' => $candidate, ':invoice' => $invoiceId]);
                $stmt = $pdo->prepare('SELECT subscription_token FROM invoice WHERE id_invoice=:invoice LIMIT 1');
                $stmt->execute([':invoice' => $invoiceId]);
                $token = strtolower(trim((string)$stmt->fetchColumn()));
            } catch (Throwable $e) {
                error_log('[subscription] token provisioning failed for invoice hash ' . substr(hash('sha256', $invoiceId), 0, 12));
                throw new RuntimeException('Subscription token storage is unavailable');
            }
            if (!preg_match('/^[a-f0-9]{64}$/', $token)) {
                throw new RuntimeException('Subscription token could not be provisioned');
            }
            $invoice['subscription_token'] = $token;
        }
        $origin = function_exists('redfox_configured_base_url') ? redfox_configured_base_url() : '';
        if ($origin === '') throw new RuntimeException('Application origin is not configured');
        return rtrim($origin, '/') . '/sub/' . $token;
    }
}
...
class ManagePanel
{
    ...
    private function loadPanel(string $key, string $column = 'name_panel'): ?array
    {
        ...
        if (is_array($value) && (string)($value['type'] ?? '') !== 'Manualsale') {
            $allowPrivate = redfox_private_panel_endpoints_allowed();
            $policy = redfox_outbound_url_policy((string)($value['url_panel'] ?? ''), $allowPrivate, $allowPrivate);
            if (empty($policy['ok'])) {
                error_log('[panel-policy] blocked configured endpoint panel_ref=' . substr(hash('sha256', (string)($value['name_panel'] ?? $key)), 0, 12));
                $value = null;
            }
        }
    ...
    function createUser($name_panel, $code_product, $usernameC, array $Data_Config)
    {
        ...
        $panelType=(string)($Get_Data_Panel['type']??'');
        if($panelType==='pasargard'){$panelType='marzban';$Get_Data_Panel['type']='marzban';$Get_Data_Panel['version_panel']='1';}
        $provisionError='';$decodedInbounds=json_decode((string)($Get_Data_Panel['inbounds']??''),true);$decodedProxies=json_decode((string)($Get_Data_Panel['proxies']??''),true);$hasInbounds=is_array($decodedInbounds)&&count($decodedInbounds)>0;$hasProxies=is_array($decodedProxies)&&count($decodedProxies)>0;
        if($panelType==='marzban'&&(!$hasInbounds||!$hasProxies))$provisionError='Marzban/Passargard inbound and proxy settings are not synchronized.';
        elseif($panelType==='marzneshin'&&!$hasProxies)$provisionError='Marzneshin service IDs are not synchronized.';
        elseif(in_array($panelType,['x-ui_single','alireza_single'],true)&&empty($Get_Data_Panel['inboundid']))$provisionError='Inbound ID is missing.';
        elseif(in_array($panelType,['ibsng','mikrotik'],true)&&empty($Get_Data_Panel['proxies']))$provisionError='Default group/profile is missing.';
        if($provisionError!=='')return ['status'=>'Unsuccessful','msg'=>$provisionError.' Open web panel → VPN panels → edit and sync a reference user.'];
        ...
        if ($Get_Data_Panel['type'] == "marzban") {
            ...
            if (!empty($data_Output['detail']) && $data_Output['detail']) {
                ...
                if ($inoice != false) {
                    $data_Output['subscription_url'] = redfox_invoice_subscription_url($inoice);
                }
            ...
            if (!empty($data_Output['error'])) {
                ...
                if (!$data_Output['success']) {
                    ...
                    $Output['configs'] = $links_user;
                    if ($inoice != false) {
                        $Output['subscription_url'] = redfox_invoice_subscription_url($inoice);
                    }
                }
            }
            ...
            if ($inoice != false) {
                $subscriptionUrl = redfox_invoice_subscription_url($inoice);
            }
            $Output['status'] = 'successful';
            $Output['username'] = $usernameC;
            $Output['subscription_url'] = $subscriptionUrl;
    ...
    function DataUser($name_panel, $username)
    {
        ...
        if ($Get_Data_Panel['type'] == "marzban") {
            ...
            if (!empty($UsernameData['error'])) {
                ...
                if ($inoice != false) {
                    $UsernameData['subscription_url'] = redfox_invoice_subscription_url($inoice);
                }
                if ((string)($Get_Data_Panel['version_panel'] ?? '0') === '1') {
            ...
            if ($inoice != false)
                $linksub = redfox_invoice_subscription_url($inoice);
            ...
            if (!isset($UsernameData)) {
                ...
                if ($inoice != false) {
                    $linksuburl = redfox_invoice_subscription_url($inoice);
                }
                $Output = array(
                    'status' => $status,
