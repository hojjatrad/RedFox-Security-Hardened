    ...
    if ($action === 'add') {
        ...
        if (!empty($errors)) {
            ...
            try {
                ...
                $flash['err'] = 'خطا: ' . redfox_public_exception($e, 'panel/discounts.php');
            }
        }
    }
    elseif ($action === 'edit') {
    ...
    elseif ($action === 'gift_delete') {
        ...
        if ($id > 0) {
            try {
                ...
                $flash['err'] = 'حذف ناموفق: ' . redfox_public_exception($e, 'panel/discounts.php');
            }
        }
    }
    ...
    elseif ($action === 'reset_usage') {
        ...
        if ($id > 0) {
            try {
                ...
                $flash['err'] = 'بازنشانی ناموفق: ' . redfox_public_exception($e, 'panel/discounts.php');
            }
        }
    }
}
...
try {
    ...
    $flash['err'] = 'بارگذاری ناموفق: ' . redfox_public_exception($e, 'panel/discounts.php');
}
...
function redfox_d_label_type($t) {
    switch ($t) {
...
function redfox_d_label_agent($a) {
    switch ($a) {
...
function redfox_d_label_section($s) {
    switch ($s) {
...
<head>
    ...
    <title>کدهای تخفیف | پنل رد فاکس</title>
                ...
                <div class="table-wrap">
                    <table id="discountsTable" class="display app-table" style="width:100%">
                        ...
                        <?php else: foreach ($list as $d):
                            ...
                            [$typeLabel, $typeBadge] = redfox_d_label_type($kind);
                            $sectionVal = $d['section'] ?? '';
                            if ($sectionVal === '') $sectionVal = in_array(($d['type'] ?? ''), ['buy','extend'], true) ? $d['type'] : 'all';
                            [$secLabel, $secBadge] = redfox_d_label_section($sectionVal);
                            [$agentLabel, $agentBadge] = redfox_d_label_agent($d['agent'] ?? '');
        ...
        <form method="POST" action="discounts.php">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" name="codeDiscount" class="form-control" style="direction:ltr; font-family:'JetBrains Mono', monospace;" placeholder="REDFOX20" required>
                </div>
                <div class="form-group">
...
document.addEventListener('DOMContentLoaded', function () {
    RedFoxDT.init('#discountsTable');
    RedFoxDT.init('#giftsTable');
});
