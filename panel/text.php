...
$textFile = dirname(__DIR__) . '/text.json';
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
    header('Content-Type: application/json; charset=utf-8');
    $contentLength = (int)($_SERVER['CONTENT_LENGTH'] ?? 0);
    if ($contentLength < 2 || $contentLength > 1048576) {
        http_response_code(413); echo json_encode(['ok'=>false,'error'=>'payload too large']); exit;
    }
    $jsonData = file_get_contents('php://input', false, null, 0, 1048577);
    $dataArray = is_string($jsonData) ? json_decode($jsonData, true, 32) : null;
    if (!is_array($dataArray) || json_last_error() !== JSON_ERROR_NONE) {
        http_response_code(400); echo json_encode(['ok'=>false,'error'=>'invalid JSON']); exit;
    }
    $encoded = json_encode($dataArray, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    if (!is_string($encoded)) {
        http_response_code(400); echo json_encode(['ok'=>false,'error'=>'unencodable JSON']); exit;
    }
    $tmp = $textFile . '.tmp.' . bin2hex(random_bytes(6));
    if (file_put_contents($tmp, $encoded . PHP_EOL, LOCK_EX) === false || !@chmod($tmp, 0640) || !@rename($tmp, $textFile)) {
        @unlink($tmp);
...
$textbot = @file_get_contents($textFile);
$textData = is_string($textbot) ? json_decode($textbot, true, 32) : null;
if (!is_array($textData)) $textData = [];
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>ویرایش متن — پنل رد فاکس</title>
...
<script>
    ...
    try {
        const data = <?php echo json_encode($textData, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT); ?>;
