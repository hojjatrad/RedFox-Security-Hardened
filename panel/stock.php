...
if (!$tableMissing && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_shelf') {
        ...
        if (isset($productByCode[$codeProduct])) {
            ...
            $productName = redfox_product_name($p);
            if ($volumeGb    <= 0) $volumeGb    = redfox_product_volume($p);
            if ($serviceDays <= 0) $serviceDays = redfox_product_days($p);
            if ($price       <= 0) $price       = redfox_product_price($p);
        }
        ...
        if (!empty($errors)) {
            ...
            if (!$flash['err']) {
                try {
                    ...
                    $flash['err'] = 'افزودن ناموفق: ' . redfox_public_exception($e, 'panel/stock.php');
                }
            }
        }
    }
    elseif ($action === 'toggle_shelf') {
        ...
        if ($id > 0) {
            try {
                ...
                $flash['err'] = 'تغییر ناموفق: ' . redfox_public_exception($e, 'panel/stock.php');
            }
        }
    }
    elseif ($action === 'delete_shelf') {
        ...
        if ($id > 0) {
            try {
                ...
                $flash['err'] = 'حذف ناموفق: ' . redfox_public_exception($e, 'panel/stock.php');
            }
        }
    }
        ...
        if (!$shelf) {
            ...
            for ($i = 0; $i < $maxRows; $i++) {
                ...
                try {
                    ...
                    $bad++;
                    error_log('[panel/stock] insert failed: ' . redfox_exception_fingerprint($e));
                }
            }
    ...
    elseif ($action === 'purge_disabled') {
        ...
        if ($shelfId > 0) {
            try {
                ...
                $flash['err'] = 'پاکسازی ناموفق: ' . redfox_public_exception($e, 'panel/stock.php');
            }
            header('Location: stock.php?shelf=' . $shelfId);
            exit;
        }
...
if (!$tableMissing) {
    try {
        ...
        $flash['err'] = $flash['err'] ?: 'بارگذاری ناموفق: ' . redfox_public_exception($e, 'panel/stock.php');
    }
    if ($viewShelfId > 0) {
...
function redfox_stock_status_label(string $s): array {
    switch ($s) {
...
<head>
    ...
    <title>انبار شبکه ملی | پنل رد فاکس</title>
            ...
            <?php if ($selectedShelf):  ?>
                ...
                <?php else: ?>
                    <?php foreach ($shelfConfigs as $cfg):
                        [$stLabel, $stClass] = redfox_stock_status_label((string)$cfg['status']);
                            ...
                            <div class="form-group">
                                ...
                                <select name="codeproduct" class="form-control" id="product-select">
                                    ...
                                    <?php foreach ($products as $p):
                                        $v = redfox_product_volume($p);
                                        $d = redfox_product_days($p);
                                        $pr = redfox_product_price($p);
                                    ?>
                                        <option value="<?php echo htmlspecialchars($p['code_product'] ?? '', ENT_QUOTES); ?>"
                                            ...
                                            <?php echo htmlspecialchars(redfox_product_name($p), ENT_QUOTES); ?>
