...
/* ---------- delete actions (POST only; central panel policy enforces CSRF) ---------- */
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
    $action = (string)($_POST['action'] ?? '');
    $deleteId = filter_var($_POST['id'] ?? null, FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]);
    if ($deleteId !== false && $action === 'delete_cancel_service') {
        try {
            $stmt = $pdo->prepare("DELETE FROM cancel_service WHERE id = :id");
            $stmt->execute([':id' => $deleteId]);
        } catch (\Throwable $e) {}
        header("Location: cancelService.php", true, 303);
        exit;
    }
    if ($deleteId !== false && $action === 'delete_resolved_payment') {
        try {
            $cur = $pdo->prepare("SELECT payment_Status FROM Payment_report WHERE id = :id LIMIT 1");
            $cur->execute([':id' => $deleteId]);
            $st = strtolower((string)$cur->fetchColumn());
            if (in_array($st, ['reject','rejected','expire','expired'], true)) {
                $pdo->prepare("DELETE FROM Payment_report WHERE id = :id")->execute([':id' => $deleteId]);
            }
        } catch (\Throwable $e) {}
        header("Location: cancelService.php", true, 303);
        exit;
...
try {
    ...
    foreach (($st ? $st->fetchAll(PDO::FETCH_ASSOC) : []) as $r) {
        ...
        if (!empty($r['id_invoice'])) {
            // پاک‌سازی آیدی فاکتور — حذف پیشوندهای getconfigafterpay| و ...
            $invId = (string)$r['id_invoice'];
            $invId = preg_replace('/^(getconfigafterpay|getconfig|miniapp_pay)\|/i', '', $invId);
            $details[] = 'فاکتور: ' . $invId;
        }
        // فقط توضیحات کوتاه و معنی‌دار را نشان بده (نه لاگ‌های طولانی)
        if (!empty($r['dec_not_confirmed'])) {
            $dec = (string)$r['dec_not_confirmed'];
            if (mb_strlen($dec) <= 80) {
                $details[] = $dec;
            }
        }
        $rows[] = [
            ...
            'del'     => in_array($stKey, ['reject','expire'], true) ? ['action' => 'delete_resolved_payment', 'id' => (int)($r['id'] ?? 0)] : null,
        ];
    }
} catch (\Throwable $e) { $flash['err'] = 'بارگذاری درخواست‌های پرداخت ناموفق: ' . redfox_public_exception($e, 'panel/cancelService.php'); }
...
try {
    ...
    foreach (($st ? $st->fetchAll(PDO::FETCH_ASSOC) : []) as $r) {
        ...
        $rows[] = [
            ...
            'date'    => '',
            'stLabel' => $stLabel, 'stClass' => $stClass, 'stKey' => $stKey,
            'del'     => ['action' => 'delete_cancel_service', 'id' => (int)($r['id'] ?? 0)],
        ];
    }
} catch (\Throwable $e) {}
...
<head>
    ...
    <title>لیست درخواست‌ها | ربات رد فاکس</title>
                        ...
                        <?php foreach ($rows as $row): ?>
                            <tr>
                                ...
                                <td data-label="عملیات" class="cell-actions">
                                    <?php if (is_array($row['del'])): ?>
                                        <form method="post" action="cancelService.php" style="display:inline" onsubmit="return confirm('این درخواست حذف شود؟')">
                                            <input type="hidden" name="action" value="<?php echo htmlspecialchars($row['del']['action'], ENT_QUOTES, 'UTF-8'); ?>">
                                            <input type="hidden" name="id" value="<?php echo (int)$row['del']['id']; ?>">
                                            <button type="submit" class="btn btn-sm btn-soft-danger"><?php echo icon('trash', 'svg-icon'); ?> حذف</button>
                                        </form>
                                    <?php else: ?>
