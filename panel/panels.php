...
// AJAX operation selectors for this page are accepted only in authenticated
// POST bodies. A query-string selector must never trigger network or DB work.
if (isset($_GET['ajax'])) {
    http_response_code(405);
    header('Allow: POST');
    header('Content-Type: application/json; charset=utf-8');
    echo json_encode(['ok' => false, 'error' => 'ajax_requires_post']);
    exit;
}
$ajaxAction = (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST')
    ? (string)($_POST['ajax'] ?? '')
    : '';
...
try {
    ...
    error_log('[panel/panels] could not read marzban_panel columns: ' . redfox_exception_fingerprint($e));
}
...
if (!function_exists('redfox_panel_authlog')) {
    function redfox_panel_authlog(string $stage, array $data = []): void {
        static $path = null;
        static $opened = false;
        if ($path === null) {
            $path = dirname(__DIR__) . '/logs/panel-auth.log';
        }
        try {
            $line = sprintf(
                "[%s] %s %s\n",
...
if (!function_exists('redfox_panel_loghost')) {
    function redfox_panel_loghost(string $url): string {
        $p = @parse_url($url);
...
if (!function_exists('redfox_panel_logfp')) {
    function redfox_panel_logfp(string $s): string {
        if ($s === '') return 'empty';
...
function redfox_curl_error_label(int $errno): string {
    return match ($errno) {
        6 => 'هاست قابل دسترس نیست (DNS)',
        7 => 'اتصال به پورت مقصد رد شد',
        28 => 'تایم‌اوت — سرور پاسخ نداد',
        35, 51, 58, 60, 77, 80, 82, 83, 90, 91 => 'خطای اعتبارسنجی TLS/SSL',
        default => 'خطای انتقال cURL (کد ' . max(0, $errno) . ')',
    };
}

function redfox_test_panel_url_reachable(string $url): array {
    if (!function_exists('curl_init')) {
        return ['ok' => false, 'code' => 0, 'error' => 'افزونه cURL در PHP فعال نیست'];
    }
    $ch = curl_init();
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_USERAGENT      => 'RedFoxPanelWeb/1.0',
    ]);
    $allowPrivate = redfox_private_panel_endpoints_allowed();
    $policy = redfox_apply_curl_url_policy($ch, $url, $allowPrivate, $allowPrivate);
    if (empty($policy['ok'])) { curl_close($ch); return ['ok' => false, 'code' => 0, 'error' => 'آدرس پنل طبق سیاست خروجی امن مجاز نیست']; }
    curl_exec($ch);
    $errno = curl_errno($ch);
    $code = (int)curl_getinfo($ch, CURLINFO_HTTP_CODE);
    curl_close($ch);
    if ($errno !== 0) {
        return ['ok' => false, 'code' => $code, 'error' => redfox_curl_error_label($errno)];
    }
    if ($code === 0 || $code >= 500) {
...
function redfox_http_post(string $url, string $body, array $headers = []): array {
    ...
    $allowPrivate = redfox_private_panel_endpoints_allowed();
    $policy = redfox_apply_curl_url_policy($ch, $url, $allowPrivate, $allowPrivate);
    if (empty($policy['ok'])) { curl_close($ch); return ['ok' => false, 'error' => 'آدرس پنل طبق سیاست خروجی امن مجاز نیست', 'code' => 0, 'body' => '', 'cookies' => []]; }
    $rawHeaders = '';
    $respBody = '';
    $responseTooLarge = false;
    $headersTooLarge = false;
    curl_setopt($ch, CURLOPT_WRITEFUNCTION, static function($curl, string $chunk) use (&$respBody, &$responseTooLarge): int {
        if (strlen($respBody) + strlen($chunk) > 2097152) { $responseTooLarge = true; return 0; }
        $respBody .= $chunk;
        return strlen($chunk);
    });
    curl_setopt($ch, CURLOPT_HEADERFUNCTION, static function($curl, string $header) use (&$rawHeaders, &$headersTooLarge): int {
        if (strlen($rawHeaders) + strlen($header) > 65536) { $headersTooLarge = true; return 0; }
        $rawHeaders .= $header;
    ...
    $code = (int)curl_getinfo($ch, CURLINFO_HTTP_CODE);
    curl_close($ch);
    if ($responseTooLarge || $headersTooLarge) return ['ok' => false, 'error' => 'پاسخ پنل بیش از حد مجاز است', 'code' => 0, 'body' => '', 'cookies' => []];
    if ($errno !== 0) return ['ok' => false, 'error' => redfox_curl_error_label($errno), 'code' => 0, 'body' => '', 'cookies' => []];
...
function redfox_http_get(string $url, array $headers = []): array {
    if (!function_exists('curl_init')) return ['ok' => false, 'error' => 'cURL در PHP نیست', 'code' => 0, 'body' => ''];
    $ch = curl_init();
    curl_setopt_array($ch, [
        CURLOPT_URL            => $url,
    ...
    $allowPrivate = redfox_private_panel_endpoints_allowed();
    $policy = redfox_apply_curl_url_policy($ch, $url, $allowPrivate, $allowPrivate);
    if (empty($policy['ok'])) { curl_close($ch); return ['ok' => false, 'error' => 'آدرس پنل طبق سیاست خروجی امن مجاز نیست', 'code' => 0, 'body' => '']; }
    $respBody = '';
    $responseTooLarge = false;
    curl_setopt($ch, CURLOPT_WRITEFUNCTION, static function($curl, string $chunk) use (&$respBody, &$responseTooLarge): int {
        if (strlen($respBody) + strlen($chunk) > 2097152) { $responseTooLarge = true; return 0; }
    ...
    $code = (int)curl_getinfo($ch, CURLINFO_HTTP_CODE);
    curl_close($ch);
    if ($responseTooLarge) return ['ok' => false, 'error' => 'پاسخ پنل بیش از حد مجاز است', 'code' => 0, 'body' => ''];
    if ($errno !== 0) return ['ok' => false, 'error' => redfox_curl_error_label($errno), 'code' => 0, 'body' => ''];
    return ['ok' => true, 'error' => '', 'code' => $code, 'body' => $respBody];
}


function redfox_test_panel_auth(string $url, string $username, string $password, string $apiKey, string $type, string $xuiToken = ''): array {
    ...
    if (in_array($type, ['x-ui_single', 'alireza_single', 's_ui'], true)) {
        if ($type === 'x-ui_single' && $xuiToken !== '') {
            ...
            if (is_array($jt) && array_key_exists('success', $jt)) {
                ...
                return ['ok' => true, 'verified' => false, 'message' => 'توکن API نامعتبر است (پاسخ success=false)'];
            }
        ...
        if (is_array($j) && array_key_exists('success', $j)) {
            ...
            return ['ok' => true, 'verified' => false, 'message' => 'یوزرنیم یا پسورد نادرست (success=false)'];
        }
        
        // A non-JSON page, redirect target, or proxy error is not proof that
        // credentials were accepted. Keep the panel inactive unless the
        // adapter observes its explicit authenticated-success contract.
        return ['ok' => true, 'verified' => false, 'message' => 'پاسخ احراز هویت معتبر دریافت نشد (HTTP ' . $r['code'] . ')'];
    }
    ...
    if (in_array($type, ['marzban', 'pasargard', 'marzneshin'], true)) {
        ...
        $tokenPath = $type === 'marzneshin' ? '/api/admins/token' : '/api/admin/token';
        $tokenPayload = ['username'=>$username,'password'=>$password]; if($type !== 'marzneshin') $tokenPayload['grant_type']='password';
        $r = redfox_http_post(
            $url . $tokenPath,
            http_build_query($tokenPayload),
        ...
        if ($r['code'] === 401 || $r['code'] === 403) {
            return ['ok' => true, 'verified' => false, 'message' => 'یوزرنیم یا پسورد نادرست (HTTP ' . $r['code'] . ')'];
        }
    ...
    if (in_array($type, ['ibsng', 'mikrotik'], true)) {
        $reach = redfox_test_panel_url_reachable($url);
        if ($reach['ok']) {
            return ['ok' => true, 'verified' => false, 'message' => 'سرور قابل دسترس است، اما احراز هویت این نوع پنل از وب پشتیبانی نمی‌شود؛ پنل خودکار فعال نشد.'];
        }
    ...
    if (($password === '' || $apiKey === '') && !empty($_POST['panel_id'])) {
        ...
        if ($pid > 0) {
            try {
                ...
                $cur->execute([':id' => $pid]);
                $row = rx_secret_decrypt_panel_row($cur->fetch(PDO::FETCH_ASSOC) ?: []);
    ...
    if ($xuiToken === '' && !empty($_POST['panel_id'])) {
        ...
        if ($pid > 0) {
            try {
                ...
                $curT->execute([':id' => $pid]);
                $rowT = rx_secret_decrypt_panel_row($curT->fetch(PDO::FETCH_ASSOC) ?: []);
                $xuiToken = (string)($rowT['xui_api_token'] ?? '');
            } catch (\Throwable $e) {}
        }
    }

    $result = redfox_test_panel_auth($url, $username, $password, $apiKey, $type, $xuiToken);
    echo json_encode($result);
    exit;
}


function redfox_sanitize_proxy_map(array $proxies): array {
    foreach ($proxies as $protocol => &$settings) {
        if (!is_array($settings)) { $settings = new stdClass(); continue; }
        unset($settings['password'], $settings['id']);
        if ($settings === []) $settings = new stdClass();
    }
    unset($settings);
    return $proxies;
}
function redfox_sync_panel_reference(PDO $pdo, string $panelName, string $referenceUsername): array {
    $q=$pdo->prepare('SELECT * FROM marzban_panel WHERE name_panel=? LIMIT 1');$q->execute([$panelName]);$panel=$q->fetch(PDO::FETCH_ASSOC);
    if(!$panel) return ['ok'=>false,'message'=>'پنل پس از ذخیره پیدا نشد.'];
    $panel=rx_secret_decrypt_panel_row($panel);$type=(string)$panel['type'];$url=rtrim((string)$panel['url_panel'],'/');
    if($referenceUsername===''||!preg_match('/^[A-Za-z0-9_.@-]{3,191}$/',$referenceUsername)) return ['ok'=>false,'message'=>'نام کاربری نمونه نامعتبر است.'];
    if($type==='marzban'){
        $login=redfox_http_post($url.'/api/admin/token',http_build_query(['username'=>$panel['username_panel'],'password'=>$panel['password_panel'],'grant_type'=>'password']),['Content-Type: application/x-www-form-urlencoded','Accept: application/json']);
        $lj=json_decode((string)$login['body'],true);$token=(string)($lj['access_token']??'');if($token==='')return ['ok'=>false,'message'=>'دریافت توکن پنل برای همگام‌سازی ناموفق بود.'];
        $r=redfox_http_get($url.'/api/user/'.rawurlencode($referenceUsername),['Accept: application/json','Authorization: Bearer '.$token]);$data=json_decode((string)$r['body'],true);if(!is_array($data)||$r['code']<200||$r['code']>=300)return ['ok'=>false,'message'=>'کاربر نمونه در پنل یافت نشد یا پاسخ نامعتبر بود.'];
        if((string)($panel['version_panel']??'0')==='1'){$inbounds=$data['group_ids']??null;$proxies=$data['proxy_settings']??null;}else{$inbounds=$data['inbounds']??null;$proxies=$data['proxies']??null;}
        if(!is_array($inbounds)||!is_array($proxies)||count($inbounds)===0||count($proxies)===0)return ['ok'=>false,'message'=>'کاربر نمونه فاقد group/inbound یا proxy settings فعال است.'];$proxies=redfox_sanitize_proxy_map($proxies);
        $pdo->prepare("UPDATE marzban_panel SET inbounds=?,proxies=?,status='active',provisioning_status='ready',provisioning_error=NULL,reference_username=? WHERE id=?")->execute([json_encode($inbounds,JSON_UNESCAPED_UNICODE),json_encode($proxies,JSON_UNESCAPED_UNICODE),$referenceUsername,$panel['id']]);
        return ['ok'=>true,'message'=>'اینباند/گروه و پروتکل‌ها از کاربر نمونه همگام شد.','inbounds'=>count($inbounds),'proxies'=>count($proxies)];
    }
    if($type==='marzneshin'){
        $login=redfox_http_post($url.'/api/admins/token',http_build_query(['username'=>$panel['username_panel'],'password'=>$panel['password_panel']]),['Content-Type: application/x-www-form-urlencoded']);$lj=json_decode((string)$login['body'],true);$token=(string)($lj['access_token']??'');if($token==='')return ['ok'=>false,'message'=>'دریافت توکن مرزنشین ناموفق بود.'];$r=redfox_http_get($url.'/api/users/'.rawurlencode($referenceUsername),['Authorization: Bearer '.$token]);$data=json_decode((string)$r['body'],true);$services=$data['service_ids']??null;if(!is_array($services)||count($services)===0)return ['ok'=>false,'message'=>'service_ids فعال از کاربر نمونه دریافت نشد.'];$pdo->prepare("UPDATE marzban_panel SET proxies=?,status='active',provisioning_status='ready',provisioning_error=NULL,reference_username=? WHERE id=?")->execute([json_encode($services),$referenceUsername,$panel['id']]);return ['ok'=>true,'message'=>'Service IDs همگام شد.'];
    }
    return ['ok'=>true,'message'=>'این نوع پنل به همگام‌سازی کاربر نمونه نیاز ندارد.'];
}

function redfox_validate_inbound_id(string $panelUrl, string $username, string $password, string $type, string $inboundId): array
{
    $url  = rtrim($panelUrl, '/');
    $id   = trim($inboundId);
    ...
    if ($type === 'WGDashboard') {
        ...
        $listResp = redfox_http_get($url . '/api/getAllPeersData', ['Authorization: Bearer ' . $token]);
    ...
    foreach ($attempts as $ep) {
        if ($ep['method'] === 'GET') {
            $listResp = redfox_http_get($url . $ep['path'], $baseHdrs);
        } else {
            $listResp = redfox_http_post(
                $url . $ep['path'],
                '',
    ...
    if (!is_array($ldata)) {
        return ['ok' => false, 'message' => 'پاسخ معتبر JSON از پنل دریافت نشد (مسیر یا session را بررسی کنید).'];
    }
    if (!isset($ldata['obj']) || !is_array($ldata['obj'])) {
        ...
        return ['ok' => false, 'message' => 'لیست اینباندها خالی یا نامعتبر است.'];
    }

    foreach ($ldata['obj'] as $inbound) {
        if (isset($inbound['id']) && (int)$inbound['id'] === $numId) {
            // Do not reflect untrusted remote remark/tag/protocol fields into
            // the administrator UI; only the locally validated numeric ID is needed.
            return ['ok' => true, 'message' => "اینباند #{$numId} یافت شد"];
        }
    }
...
function redfox_validate_sublink(string $linksubx, string $type): array
{
    $url = trim($linksubx);
...
if ($ajaxAction === 'validate_inbound') {
    header('Content-Type: application/json; charset=utf-8');
    ...
    echo json_encode(redfox_validate_inbound_id($panelUrl, $username, $password, $type, $inboundId));
    exit;
}


if ($ajaxAction === 'validate_sublink') {
    header('Content-Type: application/json; charset=utf-8');
    $linksubx = trim((string)($_POST['linksubx'] ?? ''));
    $type     = trim((string)($_POST['type']     ?? ''));
    echo json_encode(redfox_validate_sublink($linksubx, $type));
    exit;
}
...
if ($ajaxAction === 'toggle') {
    ...
    try {
        ...
        error_log('[panel/panels] toggle failed: ' . redfox_exception_fingerprint($e));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        $type      = trim((string)($_POST['type']           ?? 'marzban'));
        $requestedType = $type;
        $versionPanel = $requestedType === 'pasargard' ? '1' : '0';
        $storageType = $requestedType === 'pasargard' ? 'marzban' : $requestedType;
        $referenceUsername = trim((string)($_POST['reference_username'] ?? ''));
        $addInboundId = trim((string)($_POST['inboundid'] ?? '1'));
        $addLinkSubx = trim((string)($_POST['linksubx'] ?? $urlPanel));
        $addGroupName = trim((string)($_POST['group_name'] ?? ''));
        $agent     = (string)($_POST['agent']               ?? 'all');


        redfox_panel_authlog('ADD_START', [
            'name_fp'      => redfox_panel_logfp($namePanel),
            'type'         => $type,
            'agent'        => $agent,
            'url_fp'       => redfox_panel_logfp(redfox_panel_loghost($urlPanel)),
            'username_fp'  => redfox_panel_logfp($userPanel),
        ...
        if ($type !== 'Manualsale') {
            ...
            if ($urlPanel !== '' && filter_var($urlPanel, FILTER_VALIDATE_URL)) {
                $allowPrivatePanel = redfox_private_panel_endpoints_allowed();
                $urlPolicy = redfox_outbound_url_policy($urlPanel, $allowPrivatePanel, $allowPrivatePanel);
                if (empty($urlPolicy['ok'])) {
                    $errors[] = 'آدرس پنل طبق سیاست خروجی امن مجاز نیست. برای پنل خصوصی فقط گزینه میزبانی REDFOX_ALLOW_PRIVATE_PANEL_ENDPOINTS را فعال کنید.';
                }
            }
            ...
            if (in_array($requestedType, ['marzban','pasargard','marzneshin'], true) && !preg_match('/^[A-Za-z0-9_.@-]{3,191}$/', $referenceUsername)) {
                $errors[] = 'برای این نوع پنل، نام کاربری یک اکانت نمونه موجود در پنل لازم است تا اینباند/پروتکل/سرویس واقعاً همگام شود.';
            }
            if (in_array($requestedType, ['x-ui_single','alireza_single'], true) && !ctype_digit($addInboundId)) {
                $errors[] = 'شناسه Inbound باید عدد معتبر باشد.';
            }
            if (in_array($requestedType, ['ibsng','mikrotik'], true) && $addGroupName === '') {
                $errors[] = 'نام گروه/Profile پیش‌فرض برای این نوع پنل لازم است.';
            }
        }

        if (!empty($errors)) {
            $flash['err'] = '• ' . implode("\n• ", $errors);
            redfox_panel_authlog('ADD_VALIDATION_FAIL', ['name_fp' => redfox_panel_logfp($namePanel), 'error_count' => count($errors)]);
        } else {
            ...
            if ($type !== 'Manualsale') {
                $auth = redfox_test_panel_auth($urlPanel, $userPanel, $passPanel, $apiKey, $type, $xuiToken);


                error_log(sprintf(
                    '[panel/panels] ADD auth test: type=%s verified=%s ok=%s',
                    $type,
                    $auth['verified'] ? 'YES' : 'NO',
                    $auth['ok']       ? 'YES' : 'NO'
                ));
                redfox_panel_authlog('ADD_AUTH_TEST', [
                    'type'     => $type,
                    'url_fp'   => redfox_panel_logfp(redfox_panel_loghost($urlPanel)),
                    'username_fp' => redfox_panel_logfp($userPanel),
                ...
                if ($auth['verified'] === true) {
                    ...
                    $friendly = redfox_remote_error_summary(['message' => $rawMsg, 'http_status' => (int)($auth['http_code'] ?? 0)]);
                    $hint     = '';
            ...
            try {
                ...
                $priceDefaults=json_encode(['f'=>'4000','n'=>'4000','n2'=>'4000']);$minDefaults=json_encode(['f'=>'1','n'=>'1','n2'=>'1']);$maxDefaults=json_encode(['f'=>'1000','n'=>'1000','n2'=>'1000']);$customDefaults=json_encode(['f'=>'0','n'=>'0','n2'=>'0']);
                $stmt = $pdo->prepare("INSERT INTO marzban_panel (code_panel,name_panel,status,url_panel,username_panel,password_panel,api_key,xui_api_token,type,agent,sublink,config,MethodUsername,TestAccount,limit_panel,namecustom,Methodextend,conecton,inboundid,inbound_deactive,inboundstatus,linksubx,priceextravolume,priceextratime,pricecustomvolume,pricecustomtime,mainvolume,maxvolume,maintime,maxtime,status_extend,subvip,changeloc,customvolume,on_hold_test,version_panel,inbounds,proxies,provisioning_status,provisioning_error,reference_username) VALUES (:c,:n,:st,:u,:user,:pass,:api,:xt,:type,:agent,'onsublink','offconfig',:method_user,'ONTestAccount','unlimited','none','ریست حجم و زمان','offconecton',:inbound,'0','offinbounddisable',:linksub,:pev,:pet,:pcv,:pct,:mainv,:maxvol,:maint,:maxt,'on_extend','offsubvip','offchangeloc',:custom,'1',:version,NULL,:proxies,:prov,NULL,:reference)");
                $stmt->execute([':c'=>$codePanel,':n'=>$namePanel,':st'=>$initialStatus,':u'=>$urlPanel,':user'=>$userPanel,':pass'=>rx_secret_encrypt($passPanel),':api'=>rx_secret_encrypt($apiKey),':xt'=>rx_secret_encrypt($xuiToken),':type'=>$storageType,':agent'=>$agent,':method_user'=>'آیدی عددی + حروف و عدد رندوم',':inbound'=>$addInboundId!==''?$addInboundId:'1',':linksub'=>$addLinkSubx!==''?$addLinkSubx:$urlPanel,':pev'=>$priceDefaults,':pet'=>$priceDefaults,':pcv'=>$priceDefaults,':pct'=>$priceDefaults,':mainv'=>$minDefaults,':maxvol'=>$maxDefaults,':maint'=>$minDefaults,':maxt'=>$maxDefaults,':custom'=>$customDefaults,':version'=>$versionPanel,':proxies'=>$addGroupName!==''?$addGroupName:null,':prov'=>in_array($storageType,['marzban','marzneshin'],true)?'incomplete':'ready',':reference'=>$referenceUsername!==''?$referenceUsername:null]);
                $syncResult=['ok'=>true,'message'=>''];
                if(in_array($storageType,['marzban','marzneshin'],true)){$syncResult=redfox_sync_panel_reference($pdo,$namePanel,$referenceUsername);if(empty($syncResult['ok'])){$pdo->prepare("UPDATE marzban_panel SET status='inactive',provisioning_status='incomplete',provisioning_error=? WHERE name_panel=?")->execute([(string)$syncResult['message'],$namePanel]);$initialStatus='inactive';$testNote.=' همگام‌سازی ناقص: '.$syncResult['message'];}else{$testNote.=' '.$syncResult['message'];}}
                $statusLabel = $initialStatus === 'active' ? '«فعال»' : '«غیرفعال»';
                $flash['ok'] = 'پنل جدید با وضعیت ' . $statusLabel . ' ثبت شد.' . $testNote;
                redfox_panel_authlog('ADD_DONE', [
                    'name_fp'      => redfox_panel_logfp($namePanel),
                    'final_status' => $initialStatus,
                ]);
            } catch (\Throwable $e) {
                $flash['err'] = 'خطا در افزودن پنل: ' . redfox_public_exception($e, 'panel/panels.php');
                error_log('[panel/panels] add failed: ' . redfox_exception_fingerprint($e));
                redfox_panel_authlog('ADD_ERROR', ['name_fp' => redfox_panel_logfp($namePanel), 'error' => redfox_exception_fingerprint($e)]);
            }
        }
    }
    elseif ($action === 'edit') {
        ...
        redfox_panel_authlog('EDIT_START', [
            'id'                => $id,
            'name_fp'           => redfox_panel_logfp($namePanel),
            'type'              => $type,
            'agent'             => $agent,
            ...
            'has_password'      => $passPanel !== '',
            'has_api_key'       => $apiKey !== '',
            'pw_fp'             => redfox_panel_logfp($passPanel),
            'post_keys'         => array_keys($_POST),
        ]);

        if (!isset($PANEL_TYPES[$type])) { $type = $requestedType = $storageType = 'marzban'; $versionPanel = '0'; }
        ...
        if ($type !== 'Manualsale') {
            ...
            if ($requestedType === 'pasargard' && $referenceUsername === '') {
                $chk=$pdo->prepare('SELECT inbounds,proxies FROM marzban_panel WHERE id=?');$chk->execute([$id]);$existing=$chk->fetch(PDO::FETCH_ASSOC)?:[];
                if(empty($existing['inbounds'])||empty($existing['proxies'])) $errors[]='برای تبدیل/ویرایش پاسارگارد، نام کاربری نمونه جهت همگام‌سازی گروه و پروتکل لازم است.';
            }
        }

        if (!empty($errors)) {
            $flash['err'] = '• ' . implode("\n• ", $errors);
            redfox_panel_authlog('EDIT_VALIDATION_FAIL', ['id' => $id, 'error_count' => count($errors)]);
        } else {
            ...
            redfox_panel_authlog('EDIT_BLOCK_DECISION', [
                'id'                 => $id,
            ...
            if ($willRunAuth) {
                ...
                if ($passPanel === '' && $apiKey === '' && $xuiToken === '' && !$xuiTokenClear) {
                    try {
                        ...
                        redfox_panel_authlog('EDIT_SKIP_CHECK', [
                            'id'              => $id,
                            'pw_empty'        => true,
                            'key_empty'       => true,
                        ...
                        if ($prevWasActive && $sameUrl && $sameUser && $prevUrlValid) {
                            ...
                            error_log(sprintf(
                                '[panel/panels] EDIT auth test SKIPPED (no credential change): id=%d type=%s',
                                $id, $type
                            ));
                        }
                    } catch (\Throwable $e) {
                        redfox_panel_authlog('EDIT_SKIP_CHECK_ERROR', ['id' => $id, 'error' => redfox_exception_fingerprint($e)]);
                ...
                if ($effectivePass === '' || $effectiveKey === '' || ($effectiveToken === '' && !$xuiTokenClear)) {
                    try {
                        ...
                        $cur = rx_secret_decrypt_panel_row($curStmt->fetch(PDO::FETCH_ASSOC) ?: []);
                ...
                redfox_panel_authlog('EDIT_AUTH_RUNNING', [
                    'id'             => $id,
                    'type'           => $type,
                    'url_fp'         => redfox_panel_logfp(redfox_panel_loghost($urlPanel)),
                    'username_fp'    => redfox_panel_logfp($userPanel),
                    'effective_pw_fp'  => redfox_panel_logfp($effectivePass),
                    'effective_key_fp' => redfox_panel_logfp($effectiveKey),
                    'pw_source'        => $passPanel !== '' ? 'form' : 'db',
                    'key_source'       => $apiKey   !== '' ? 'form' : 'db',
                ]);

                $auth = redfox_test_panel_auth($urlPanel, $userPanel, $effectivePass, $effectiveKey, $type, $effectiveToken);


                error_log(sprintf(
                    '[panel/panels] EDIT auth test: id=%d type=%s verified=%s ok=%s',
                    $id,
                    $type,
                    $auth['verified'] ? 'YES' : 'NO',
                    $auth['ok']       ? 'YES' : 'NO'
                ));
                redfox_panel_authlog('EDIT_AUTH_TEST', [
                    'id'       => $id,
                    'type'     => $type,
                    'url_fp'   => redfox_panel_logfp(redfox_panel_loghost($urlPanel)),
                    'username_fp' => redfox_panel_logfp($userPanel),
            ...
            try {
                $sets = ["name_panel = :n", "url_panel = :u", "username_panel = :user", "type = :type", "version_panel = :version_panel", "agent = :agent", "status = :status"];
                $params = [
                    ':n'      => $namePanel,
                    ':u'      => $urlPanel,
                    ':user'   => $userPanel,
                ...
                if (!$authFailed) {
                    if ($passPanel !== '') { $sets[] = "password_panel = :pass"; $params[':pass'] = rx_secret_encrypt($passPanel); }
                    if ($apiKey    !== '') { $sets[] = "api_key = :api";          $params[':api']  = rx_secret_encrypt($apiKey);    }
                }
                if ($xuiTokenClear) {
                    $sets[] = "xui_api_token = :xt"; $params[':xt'] = '';
                } elseif ($xuiToken !== '') {
                    $sets[] = "xui_api_token = :xt"; $params[':xt'] = rx_secret_encrypt($xuiToken);
                }
                ...
                $stmt->execute($params);
                if ($referenceUsername !== '' && in_array($storageType,['marzban','marzneshin'],true)) {
                    $syncResult=redfox_sync_panel_reference($pdo,$namePanel,$referenceUsername);
                    if(empty($syncResult['ok'])){$pdo->prepare("UPDATE marzban_panel SET status='inactive',provisioning_status='incomplete',provisioning_error=? WHERE id=?")->execute([(string)$syncResult['message'],$id]);$testNote.=' همگام‌سازی ناقص: '.$syncResult['message'];}
                    else{$testNote.=' '.$syncResult['message'];}
                }
                $flash['ok'] = 'پنل به‌روزرسانی شد.' . $testNote;
                redfox_panel_authlog('EDIT_DONE', [
                    'id'                  => $id,
                ...
                $flash['err'] = 'خطا در به‌روزرسانی: ' . redfox_public_exception($e, 'panel/panels.php');
                error_log('[panel/panels] edit failed: ' . redfox_exception_fingerprint($e));
                redfox_panel_authlog('EDIT_ERROR', ['id' => $id, 'error' => redfox_exception_fingerprint($e)]);
            }
        }
    }
    elseif ($action === 'delete') {
        ...
        if ($id <= 0) {
            ...
            try {
                ...
                $flash['err'] = 'حذف ناموفق: ' . redfox_public_exception($e, 'panel/panels.php');
            }
        }
    }
        ...
        if ($id <= 0) {
            ...
            if ($sourceOk) {
                try {
                    ...
                    $flash['err'] = 'ثبت ناموفق: ' . redfox_public_exception($e, 'panel/panels.php');
                }
            }
        }
    }
...
try {
    ...
    $flash['err'] = $flash['err'] ?: ('بارگذاری ناموفق: ' . redfox_public_exception($e, 'panel/panels.php'));
}

function redfox_mask_secret($v) {
    $v = (string)$v;
    $n = strlen($v);
...
function redfox_is_truthy_panel_flag($v, $trueValues) {
...
<head>
    ...
    <title>مدیریت پنل‌ها | پنل رد فاکس</title>
        ...
        <div class="wrapper">
            ...
            <?php else: ?>
                ...
                <?php foreach ($panels as $p):
                    ...
                    if ($type === 'marzban' && (string)($p['version_panel'] ?? '0') === '1') $type = 'pasargard';
                    $typeLabel   = $PANEL_TYPES[$type] ?? $type;
                    $emergencyOn = redfox_is_truthy_panel_flag($p['emergency_panel_status'] ?? '', ['on_emergency_panel']);
                    $nationalOn  = redfox_is_truthy_panel_flag($p['national_net_status']    ?? '', ['on_national_net']);
                    $testOn      = redfox_is_truthy_panel_flag($p['TestAccount']            ?? '', ['ONTestAccount']);
                    $inboundOn   = redfox_is_truthy_panel_flag($p['inboundstatus']          ?? '', ['offinbounddisable']);
                ?>
                    <div class="panel-card">
                        <div class="panel-card__head">
                            <div class="panel-card__name">
                                ...
                                <?php endif; ?>
                                <?php if (($p['provisioning_status'] ?? 'incomplete') !== 'ready' && in_array((string)($p['type']??''),['marzban','marzneshin','pasargard'],true)): ?>
                                    <small class="badge badge-block" title="<?= htmlspecialchars((string)($p['provisioning_error']??'نیازمند همگام‌سازی'),ENT_QUOTES) ?>">پیکربندی ناقص</small>
                                <?php endif; ?>
                            </div>
                            <div class="panel-card__actions">
                                <button class="btn btn-sm btn-soft-purple"
                                        onclick='openEdit(<?php echo json_encode([
                                            ...
                                            "reference_username" => $p["reference_username"] ?? "",
                                            "has_password"    => !empty($p["password_panel"]),
                        ...
                        <div class="panel-card__body">
                            <div class="panel-card__info">
                                <?php if ($type !== 'Manualsale'): ?>
                                    ...
                                    <div class="info-line">
                                        <span class="info-line__k">Provisioning:</span>
                                        <span class="info-line__v"><?= htmlspecialchars((string)($p['provisioning_status']??'incomplete'),ENT_QUOTES) ?><?= !empty($p['reference_username'])?' · ref: '.htmlspecialchars((string)$p['reference_username'],ENT_QUOTES):'' ?></span>
                                    </div>
                                    <?php if(!empty($p['provisioning_error'])): ?><div class="info-line"><span class="info-line__k">خطا:</span><span class="info-line__v" style="color:var(--danger)"><?= htmlspecialchars((string)$p['provisioning_error'],ENT_QUOTES) ?></span></div><?php endif; ?>
                                    <div class="info-line">
                                        <span class="info-line__k">پسورد:</span>
                                        <span class="info-line__v"><?php echo redfox_mask_secret($p['password_panel'] ?? ''); ?></span>
                                    </div>
                                    <?php if ($type === 'x-ui_single' && trim((string)($p['xui_api_token'] ?? '')) !== ''): ?>
                                    <div class="info-line">
                                        <span class="info-line__k">توکن 3x-ui:</span>
                                        <span class="info-line__v"><?php echo redfox_mask_secret($p['xui_api_token'] ?? ''); ?> <small style="color:var(--success,#16a34a)">(حالت توکنی فعال)</small></span>
                                    </div>
                                    <?php endif; ?>
                                <?php else: ?>
                            ...
                            <div class="panel-card__toggles">
                                ...
                                $configOn = redfox_is_truthy_panel_flag($p['config'] ?? '', ['onconfig']);
                                ...
                                $sublinkOn = redfox_is_truthy_panel_flag($p['sublink'] ?? '', ['onsublink']);
                ...
                <div class="card view-pane" data-view="table" style="display:none;">
                    ...
                    if ($type === 'marzban' && (string)($p['version_panel'] ?? '0') === '1') $type = 'pasargard';
                                ...
                                $nationalOn  = redfox_is_truthy_panel_flag($p['national_net_status']    ?? '', ['on_national_net']);
                                $testOn      = redfox_is_truthy_panel_flag($p['TestAccount']            ?? '', ['ONTestAccount']);
                                $configOn_t  = redfox_is_truthy_panel_flag($p['config']    ?? '', ['onconfig']);
                                $sublinkOn_t = redfox_is_truthy_panel_flag($p['sublink']   ?? '', ['onsublink']);
                                $conectonOn_t= redfox_is_truthy_panel_flag($p['conecton']  ?? '', ['onconecton']);
                            ?>
                                <tr>
        ...
        <form method="POST" action="panels.php">
            ...
            <div class="form-row" id="addCredsGroup">
                ...
                <div class="form-group">
                    ...
                    <input type="password" name="password_panel" id="password_panel" class="form-control" style="direction:ltr" autocomplete="new-password">
                </div>
            </div>
            <div class="form-group">
                <label class="form-label">کاربر نمونه برای همگام‌سازی واقعی</label>
                <input type="text" name="reference_username" class="form-control" style="direction:ltr" placeholder="یک username موجود در Marzban/Passargard/Marzneshin">
                <small>برای Marzban، پاسارگارد و Marzneshin اجباری است؛ گروه‌ها، اینباندها و پروتکل‌ها از همین کاربر خوانده می‌شوند.</small>
            </div>
            <div class="form-row">
                <div class="form-group"><label class="form-label">Inbound ID</label><input type="text" name="inboundid" value="1" class="form-control" style="direction:ltr"><small>برای x-ui تک‌پورت عدد واقعی Inbound</small></div>
                <div class="form-group"><label class="form-label">لینک Subscription</label><input type="text" name="linksubx" class="form-control" style="direction:ltr" placeholder="خالی = URL پنل"></div>
            </div>
            <div class="form-group"><label class="form-label">نام Group/Profile</label><input type="text" name="group_name" class="form-control" style="direction:ltr" placeholder="برای IBSng یا MikroTik"></div>
        ...
        <form method="POST" action="panels.php">
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <input type="password" name="password_panel" id="edit_password_panel" class="form-control" style="direction:ltr" autocomplete="new-password" placeholder="برای تغییر پسورد، اینجا تایپ کنید">
                </div>
            </div>
            <div class="form-group">
                <label class="form-label">همگام‌سازی مجدد با کاربر نمونه</label>
                <input type="text" name="reference_username" id="edit_reference_username" class="form-control" style="direction:ltr" placeholder="اختیاری؛ با واردکردن username، inbound/proxy/service مجدداً خوانده می‌شود">
            </div>
...
function testPanelAuth(mode) {
    ...
    fetch('panels.php', { method: 'POST', body: fd, credentials: 'same-origin' })
...
function openEdit(d) {
    ...
    setIfExists('edit_reference_username', d.reference_username);
    setIfExists('edit_xui_api_token',   '');

    // Red Fox: نشانگر پسورد ذخیره‌شده
    var pwField = document.getElementById('edit_password_panel');
    if (pwField) {
        pwField.value = ''; // همیشه خالی شروع کن
        pwField.placeholder = d.has_password ? '•••••••• رمز ذخیره شده — برای تغییر تایپ کنید' : 'رمز عبور را وارد کنید';
    }
...
(function () {
    var STORE_KEY = 'redfox_panels_view';
