...
register_shutdown_function(static function () {
    ...
    error_log('[panel-index fatal] type ' . (int)($err['type'] ?? 0)
        . ' @ ' . basename((string)($err['file'] ?? 'unknown')) . ':' . (int)($err['line'] ?? 0));
...
function redfox_chart_data(\PDO $pdo, string $range, int $customDays = 7): array
{
    $now = time();
...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'chart') {
    ...
    try {
        $payload = redfox_chart_data($pdo, $range, $days);
        echo json_encode(['ok' => true] + $payload, JSON_UNESCAPED_UNICODE);
    } catch (\Throwable $e) {
        error_log('[panel/dashboard] chart query failed: ' . redfox_exception_fingerprint($e));
        http_response_code(500);
        echo json_encode(['ok' => false, 'error' => 'chart_failed']);
    }
    exit;
}
...
$initialChart = redfox_chart_data($pdo, '7d');
...
<head>
    ...
    <title>پنل مدیریت ربات رد فاکس</title>
...
(function () {
    ...
    document.addEventListener('red fox:themechange', function () {
        if (typeof RedFoxChart !== 'undefined') renderChart();
    });
})();
</script>
<?php endif; ?>
