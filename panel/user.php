...
$targetUid = trim((string)($_GET['id'] ?? ''));
if (!preg_match('/^[0-9]{1,20}$/', $targetUid)) {
    http_response_code(400);
    exit('شناسه کاربر نامعتبر است');
}
$query = $pdo->prepare("SELECT * FROM user WHERE id=:id LIMIT 1");
$query->execute([':id' => $targetUid]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['_action'])) {
    ...
    if ($act === 'user_code_delete' && $targetUid !== '') {
        ...
        header("Location: user.php?id=" . urlencode($targetUid), true, 303);
        exit;
    }
    if ($act === 'manage_status') {
        $newStatus = (string)($_POST['status'] ?? '');
        if (!in_array($newStatus, ['active', 'block'], true)) { http_response_code(400); exit('وضعیت نامعتبر است'); }
        $st = $pdo->prepare('UPDATE user SET User_Status=:status WHERE id=:id');
        $st->execute([':status'=>$newStatus, ':id'=>$targetUid]);
        if ($newStatus === 'block') {
            $reportText = 'کاربر با آیدی عددی ' . $targetUid . " در ربات مسدود گردید\n\nادمین مسدود کننده: " . (string)$_SESSION['user'];
            if (!empty($setting['Channel_Report'])) {
                telegram('sendmessage', ['chat_id'=>$setting['Channel_Report'], 'message_thread_id'=>$otherservice, 'text'=>$reportText]);
            }
        } else {
            sendmessage($targetUid, "✳️ حساب کاربری شما از مسدودی خارج شد ✳️\nاکنون می‌توانید از ربات استفاده کنید.", null, null);
        }
        header("Location: user.php?id=" . urlencode($targetUid), true, 303); exit;
    }
    if (in_array($act, ['balance_add', 'balance_subtract'], true)) {
        $amountRaw = trim((string)($_POST['amount'] ?? ''));
        if (!ctype_digit($amountRaw) || (int)$amountRaw < 1 || (int)$amountRaw > 100000000) {
            http_response_code(400); exit('مبلغ نامعتبر است');
        }
        $amount = (int)$amountRaw;
        try {
            $pdo->beginTransaction();
            if ($act === 'balance_add') {
                $balanceUpdate = $pdo->prepare('UPDATE user SET Balance=Balance+:amount WHERE id=:id');
                $method = 'add balance by admin';
            } else {
                $balanceUpdate = $pdo->prepare('UPDATE user SET Balance=Balance-:amount WHERE id=:id AND Balance>=:amount_guard');
                $method = 'low balance by admin';
            }
            $params = [':amount'=>$amount, ':id'=>$targetUid];
            if ($act === 'balance_subtract') $params[':amount_guard'] = $amount;
            $balanceUpdate->execute($params);
            if ($balanceUpdate->rowCount() !== 1) throw new RuntimeException('balance update rejected');
            $orderId = 'panel_' . bin2hex(random_bytes(12));
            $paymentInsert = $pdo->prepare("INSERT INTO Payment_report (id_user,id_order,time,price,payment_Status,Payment_Method,id_invoice) VALUES (:uid,:oid,:time,:price,'paid',:method,NULL)");
            $paymentInsert->execute([':uid'=>$targetUid, ':oid'=>$orderId, ':time'=>date('Y/m/d H:i:s'), ':price'=>$amount, ':method'=>$method]);
            $pdo->commit();
        } catch (Throwable $e) {
            if ($pdo->inTransaction()) $pdo->rollBack();
            http_response_code(409); exit('تغییر موجودی انجام نشد');
        }
        $formatted = number_format($amount);
        $userMessage = $act === 'balance_add'
            ? "💎 کاربر عزیز مبلغ {$formatted} تومان به موجودی کیف پول شما اضافه شد."
            : "❌ کاربر عزیز مبلغ {$formatted} تومان از موجودی کیف پول شما کسر شد.";
        sendmessage($targetUid, $userMessage, null, null);
        if (!empty($setting['Channel_Report'])) {
            $verb = $act === 'balance_add' ? 'افزایش' : 'کاهش';
            $reportText = "📌 موجودی کاربر از پنل {$verb} یافت.\n\nادمین: " . (string)$_SESSION['user'] . "\nکاربر: {$targetUid}\nمبلغ: {$formatted}";
            telegram('sendmessage', ['chat_id'=>$setting['Channel_Report'], 'message_thread_id'=>$paymentreports, 'text'=>$reportText]);
        }
        header("Location: user.php?id=" . urlencode($targetUid), true, 303); exit;
    }
    if ($act === 'change_agent') {
        $agent = (string)($_POST['agent'] ?? '');
        if (!in_array($agent, ['f','n','n2'], true)) { http_response_code(400); exit('سطح کاربری نامعتبر است'); }
        $pdo->prepare('UPDATE user SET agent=:agent WHERE id=:id')->execute([':agent'=>$agent, ':id'=>$targetUid]);
        header("Location: user.php?id=" . urlencode($targetUid), true, 303); exit;
    }
    if ($act === 'send_message') {
        $message = trim((string)($_POST['textmessage'] ?? ''));
        if ($message === '' || mb_strlen($message) > 3500) { http_response_code(400); exit('متن پیام نامعتبر است'); }
        $outbound = "📥 یک پیام از مدیریت برای شما ارسال شد.\n\nمتن پیام: " . $message;
        sendmessage($targetUid, $outbound, null, null);
        if (!empty($setting['Channel_Report'])) {
            $reportText = "📌 پیام مدیریت ارسال شد\n\nادمین: " . (string)$_SESSION['user'] . "\nگیرنده: {$targetUid}\nمتن: {$message}";
            telegram('sendmessage', ['chat_id'=>$setting['Channel_Report'], 'message_thread_id'=>$otherservice, 'text'=>$reportText]);
        }
        header("Location: user.php?id=" . urlencode($targetUid), true, 303); exit;
    }
        ...
        <div class="wrapper">
            ...
            <div class="card">
                ...
                <div class="actions-grid">
                    <form method="post" action="user.php?id=<?php echo urlencode((string)$user['id']); ?>"<?php echo $user['User_Status'] === 'block' ? '' : ' onsubmit="return confirm(\'کاربر مسدود شود؟\')"'; ?>>
                        <input type="hidden" name="_csrf" value="<?php echo htmlspecialchars($_csrf, ENT_QUOTES, 'UTF-8'); ?>">
                        <input type="hidden" name="_action" value="manage_status">
                        <input type="hidden" name="status" value="<?php echo $user['User_Status'] === 'block' ? 'active' : 'block'; ?>">
                        <button type="submit" class="btn <?php echo $user['User_Status'] === 'block' ? 'btn-soft-success' : 'btn-soft-danger'; ?> btn-block">
                            <?php echo $user['User_Status'] === 'block' ? icon('check', 'svg-icon') . ' رفع مسدودی' : icon('ban', 'svg-icon') . ' مسدود کردن'; ?>
                        </button>
                    </form>
                    ...
                    <form method="post" action="user.php?id=<?php echo urlencode((string)$user['id']); ?>" onsubmit="return confirm('آیا مطمئن هستید؟')">
                        <input type="hidden" name="_csrf" value="<?php echo htmlspecialchars($_csrf, ENT_QUOTES, 'UTF-8'); ?>">
                        <input type="hidden" name="_action" value="change_agent">
                        <input type="hidden" name="agent" value="f">
                        <button type="submit" class="btn btn-soft-danger btn-block"><?php echo icon('user-xmark', 'svg-icon'); ?> حذف نمایندگی</button>
                    </form>

                    <a href="user_service.php?uid=<?php echo urlencode((string)$user['id']); ?>" class="btn btn-primary btn-block" style="margin-top:8px">
                        🔌 سرویس‌ها و کانفیگ
                    </a>
...
<div id="modal-add-balance" class="modal-overlay">
    <div class="modal-box">
        ...
        <form action="user.php?id=<?php echo urlencode((string)$user['id']); ?>" method="POST">
            <input type="hidden" name="_csrf" value="<?php echo htmlspecialchars($_csrf, ENT_QUOTES, 'UTF-8'); ?>">
            <input type="hidden" name="_action" value="balance_add">
            <div class="form-group">
                <label class="form-label">مبلغ (تومان)</label>
                <input type="number" name="amount" min="1" max="100000000" class="form-control" placeholder="مثلا 50000" required>
            </div>
...
<div id="modal-low-balance" class="modal-overlay">
    <div class="modal-box">
        ...
        <form action="user.php?id=<?php echo urlencode((string)$user['id']); ?>" method="POST">
            <input type="hidden" name="_csrf" value="<?php echo htmlspecialchars($_csrf, ENT_QUOTES, 'UTF-8'); ?>">
            <input type="hidden" name="_action" value="balance_subtract">
            <div class="form-group">
                <label class="form-label">مبلغ کسر (تومان)</label>
                <input type="number" name="amount" min="1" max="100000000" class="form-control" placeholder="مثلا 10000" required>
            </div>
