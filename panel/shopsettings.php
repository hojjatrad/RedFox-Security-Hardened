...
if (!$tableMissing) {
    try {
        ...
        error_log('[panel/shopsettings] load failed: ' . redfox_exception_fingerprint($e));
    }
}
    ...
    foreach ($SHOP_GROUPS as $group) {
        foreach ($group['fields'] as $f) {
            ...
            try {
                ...
                error_log('[panel/shopsettings] save ' . $key . ' failed: ' . redfox_exception_fingerprint($e));
            }
        }
    }
...
function redfox_shop_is_on($cur, $on, $off) {
...
<head>
    ...
    <title>قابلیت‌های فروشگاه | پنل رد فاکس</title>
                                            ...
                                            <?php if ($f['type'] === 'toggle'): ?>
                                                <label class="switch" title="<?php echo htmlspecialchars($key); ?>">
                                                    <input type="checkbox" id="<?php echo $idAttr; ?>" name="<?php echo $idAttr; ?>"
                                                        <?php echo redfox_shop_is_on($cur, $f['on'], $f['off']) ? 'checked' : ''; ?>>
