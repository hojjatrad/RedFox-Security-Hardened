...
$FIN_GROUPS = [
    ...
    'crypto_gateways' => [
        ...
        'fields' => [
            ...
            ['type' => 'secret', 'name' => 'nowpayment_ipn_secret','label' => 'IPN Secret NowPayments (الزامی)', 'placeholder' => 'حداقل ۱۶ نویسه؛ دقیقاً مطابق داشبورد NowPayments'],
            ['type' => 'secret', 'name' => 'tronado_webhook_secret','label' => 'HMAC Secret ترونادو (الزامی)', 'placeholder' => 'حداقل ۱۶ نویسه؛ مطابق مستندات امضای webhook درگاه'],
...
try {
    ...
    error_log('[panel/finance] load failed: ' . redfox_exception_fingerprint($e));
}
...
try {
    ...
    error_log('[panel/finance] textbot load failed: ' . redfox_exception_fingerprint($e));
}
        ...
        foreach ($group['fields'] as $f) {
            ...
            if ($fieldType === 'textbot') {
                ...
                try {
                    ...
                    error_log('[panel/finance] save textbot ' . $key . ' failed: ' . redfox_exception_fingerprint($e));
                }
                continue;
            }
            ...
            try {
                ...
                error_log('[panel/finance] save ' . $key . ' failed: ' . redfox_exception_fingerprint($e));
            }
        }
    }
...
function redfox_fin_is_on($cur, $on, $off) {
...
function redfox_fin_mask_secret($v) {
    $v = (string)$v;
    $n = strlen($v);
    if ($n === 0) return '';
...
<head>
    ...
    <title>تنظیمات مالی | پنل رد فاکس</title>
                                    ...
                                    <div class="setting-row__control">
                                        <?php if ($f['type'] === 'toggle'): ?>
                                            <label class="switch" title="<?php echo htmlspecialchars($key); ?>">
                                                <input type="checkbox" id="<?php echo $idAttr; ?>" name="<?php echo $idAttr; ?>"
                                                    <?php echo redfox_fin_is_on($cur, $f['on'], $f['off']) ? 'checked' : ''; ?>>
                                        ...
                                        <?php elseif ($f['type'] === 'secret'): ?>
                                            <input type="text" id="<?php echo $idAttr; ?>" name="<?php echo $idAttr; ?>"
                                                value=""
                                                placeholder="<?php echo htmlspecialchars(redfox_fin_mask_secret($cur) ?: ($f['placeholder'] ?? ''), ENT_QUOTES); ?>"
