...
require_once __DIR__ . '/../lib/MessageTemplates.php';
...
$templateDefs=rx_message_template_definitions();try{$seed=$pdo->prepare('INSERT IGNORE INTO textbot(id_text,text) VALUES(?,?)');foreach($templateDefs as$k=>$def)$seed->execute([$k,$def['default']]);}catch(Throwable$e){error_log('[textbot templates] '.redfox_exception_fingerprint($e));}

function redfox_text_group(string $key): string {
    $k = strtolower($key);
    if (strpos($k,'report_reseller_')===0) return 'گزارش‌های خرید و سرویس نماینده';
    if (strpos($k,'alert_reseller_')===0) return 'هشدارهای اعتبار نماینده';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'restore_templates') {
        try { $seed=$pdo->prepare('INSERT IGNORE INTO textbot(id_text,text) VALUES(?,?)');$n=0;foreach($templateDefs as$k=>$def){$seed->execute([$k,$def['default']]);$n+=$seed->rowCount();}header('Location:textbot.php?templates_restored='.$n.'#operational-templates');exit; }
        catch(Throwable$e){$flash['err']='بازسازی قالب‌ها ناموفق بود: '.redfox_public_exception($e, 'panel/textbot.php');}
    }
    elseif ($action === 'save') {
        $newValues = $_POST['t'] ?? [];
        ...
        foreach ($newValues as $key => $val) {
            $key = (string)$key;
            $val = (string)$val;
            if (!isset($cur[$key]) && !isset($templateDefs[$key])) continue;
            if (isset($cur[$key]) && $cur[$key] === $val) continue;
            try {
                $upd = $pdo->prepare("INSERT INTO textbot(id_text,text) VALUES(:k,:v) ON DUPLICATE KEY UPDATE text=VALUES(text)");
                $upd->execute([':v' => $val, ':k' => $key]);
                $updated++;
            } catch (\Throwable $e) {
                $flash['err'] = redfox_public_exception($e, 'panel/textbot.php');
                error_log('[panel/textbot] update failed for ' . $key . ': ' . redfox_exception_fingerprint($e));
            }
        }
    ...
    elseif ($action === 'add') {
        ...
        if (!empty($errors)) {
            ...
            try {
                ...
                $flash['err'] = 'افزودن ناموفق: ' . redfox_public_exception($e, 'panel/textbot.php');
            }
        }
    }
    elseif ($action === 'delete') {
        ...
        if ($key !== '') {
            try {
                ...
                $flash['err'] = 'حذف ناموفق: ' . redfox_public_exception($e, 'panel/textbot.php');
            }
        }
    }
}
...
try {
    ...
    $flash['err'] = 'بارگذاری متن‌ها ناموفق: ' . redfox_public_exception($e, 'panel/textbot.php');
}
// قالب‌های سیستمی حتی قبل از Migration نیز در UI دیده می‌شوند و هنگام ذخیره Upsert خواهند شد.
$existingKeys=[];foreach($rows as$r)$existingKeys[(string)$r['id_text']]=true;foreach($templateDefs as$k=>$def)if(!isset($existingKeys[$k]))$rows[]=['id_text'=>$k,'text'=>$def['default'],'_virtual'=>1];

$grouped = [];
foreach ($rows as $row) {
    $g = redfox_text_group((string)$row['id_text']);
...
$preferredOrder = [
    ...
    'گزارش‌های خرید و سرویس نماینده',
    'هشدارهای اعتبار نماینده',
    'تخفیف و کیف پول',
    'متن‌های پرداخت',
    'متن‌های بعد از خرید',
...
$restoredNum=isset($_GET['templates_restored'])?(int)$_GET['templates_restored']:-1;
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>متن‌های ربات | پنل رد فاکس</title>
...
<section id="container">
    ...
    <section id="main-content">
        <div class="wrapper">
            ...
            <div class="card" id="operational-templates" style="border:1px solid var(--accent-mid);background:var(--accent-soft)"><h2 style="margin:0 0 8px">قالب‌های گزارش و اعلان قابل ویرایش</h2><a class="btn btn-primary" href="message_templates.php" style="margin-bottom:10px">بازکردن ویرایشگر اختصاصی قالب‌های نماینده</a><p class="page-guide">گزارش خرید، تمدید، اکانت تست و هشدارهای اعتبار نماینده در همین صفحه قرار دارند. با کلیک روی هر عنوان، در جستجو نمایش داده می‌شود.</p><div style="display:flex;gap:6px;flex-wrap:wrap"><?php foreach($templateDefs as$tk=>$td):?><button type="button" class="btn btn-sm" onclick="document.getElementById('searchBox').value='<?=htmlspecialchars($tk,ENT_QUOTES)?>';document.getElementById('searchBox').dispatchEvent(new Event('input'));document.getElementById('searchBox').scrollIntoView({behavior:'smooth'})"><?=htmlspecialchars($td['title'])?></button><?php endforeach;?></div><form method="post" style="margin-top:10px"><input type="hidden" name="_action" value="restore_templates"><button class="btn btn-primary btn-sm">بازسازی قالب‌های مفقود در دیتابیس</button></form></div>
            <?php if($restoredNum>=0):?><div class="alert" style="background:var(--color-success-soft);padding:12px;border-radius:10px">تعداد <?=$restoredNum?> قالب مفقود به دیتابیس اضافه شد.</div><?php endif;?>
                    ...
                    <div class="text-grp">
                        ...
                        ?>
                            <div class="text-row" data-search="<?php echo htmlspecialchars(strtolower($k . ' ' . $v), ENT_QUOTES); ?>">
                                <div><code class="text-key"><?php echo htmlspecialchars($k, ENT_QUOTES); ?></code><?php if(isset($templateDefs[$k])):?><small style="display:block;color:var(--text-muted);line-height:1.8;margin-top:5px"><?=htmlspecialchars($templateDefs[$k]['title'])?><br><b>متغیرها:</b> <?=htmlspecialchars(implode('، ',array_map(fn($v)=>'{'.$v.'}',$templateDefs[$k]['vars'])))?></small><?php endif;?></div>
