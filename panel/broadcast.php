...
function redfox_target_sql(string $target): array {
    switch ($target) {
        case 'active':
            return [" WHERE (User_Status IS NULL OR User_Status != 'block')", []];
        case 'agents':
            return [" WHERE agent IN ('n', 'n2')", []];
        case 'all_users':
        default:
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'GET' && $ajaxGet === 'count') {
    ...
    [$whereSql, $params] = redfox_target_sql($target);
    try {
        $stmt = $pdo->prepare('SELECT COUNT(*) FROM user' . $whereSql);
        $stmt->execute($params);
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST' && $ajaxPost === 'send_batch') {
    header('Content-Type: application/json; charset=utf-8');
    header('Cache-Control: no-store');
    @set_time_limit(20);

    $target = (string)($_POST['target'] ?? 'all_users');
    $offsetRaw = trim((string)($_POST['offset'] ?? '0'));
    $batchRaw = trim((string)($_POST['batch'] ?? '15'));
    ...
    if (!preg_match('/^[0-9]{1,9}$/', $offsetRaw) || !preg_match('/^(?:[1-9]|[12][0-9]|30)$/', $batchRaw)) {
        http_response_code(422);
        echo json_encode(['ok' => false, 'error' => 'pagination_invalid']);
        exit;
    }
    $offset = min(100000000, (int)$offsetRaw);
    $batch = (int)$batchRaw;
    if (!in_array($parse, ['HTML', 'Markdown', 'plain'], true)) $parse = 'HTML';
    ...
    foreach ($rows as $row) {
        ...
        if ($uid <= 0) continue;
        try {
            $result = sendmessage($uid, $message, null, $parseMode);
            if (is_array($result) && !empty($result['ok'])) $sent++; else $failed++;
        } catch (\Throwable $e) {
            $failed++;
        }
    }
...
<head>
    ...
    <title>پیام همگانی | پنل رد فاکس</title>
...
(function () {
    ...
    function runJob(total) {
        ...
        function tick() {
            var fd = new FormData();
            fd.append('ajax', 'send_batch');
            fd.append('target', elTarget.value);
            fd.append('offset', String(offset));
            fd.append('batch', '15');
            fd.append('message', elMessage.value);
            fd.append('parse', elParse.value);
            fetch('broadcast.php', { method: 'POST', body: fd, credentials: 'same-origin' })
                .then(function (r) { return r.json(); })
                .then(function (j) {
