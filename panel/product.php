...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && ($_POST['action'] ?? '') === 'reorder') {
    $first = filter_var($_POST['oneproduct'] ?? null, FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]);
    $second = filter_var($_POST['toweproduct'] ?? null, FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]);
    ...
    try {
        ...
        $check = $pdo->prepare('SELECT id FROM product WHERE id IN (:first,:second) FOR UPDATE');
        $check->execute([':first' => $first, ':second' => $second]);
        if (count($check->fetchAll(PDO::FETCH_COLUMN)) !== 2) throw new RuntimeException('product missing');
        $temporary = (int)$pdo->query('SELECT COALESCE(MAX(id),0)+1 FROM product')->fetchColumn();
        $swap = $pdo->prepare('UPDATE product SET id=:new_id WHERE id=:old_id');
        $swap->execute([':new_id' => $temporary, ':old_id' => $first]);
        $swap->execute([':new_id' => $first, ':old_id' => $second]);
        $swap->execute([':new_id' => $second, ':old_id' => $temporary]);
        $pdo->commit();
    } catch (Throwable $e) {
        if ($pdo->inTransaction()) $pdo->rollBack();
        http_response_code(409);
        exit('جابجایی محصول انجام نشد');
    }
...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && ($_POST['action'] ?? '') === 'delete') {
    $removeId = filter_var($_POST['id'] ?? null, FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]);
    if ($removeId === false) { http_response_code(400); exit('شناسه نامعتبر است'); }
...
<head>
    ...
    <title>مدیریت محصولات | ربات رد فاکس</title>
                            ...
                            <tr>
                                ...
                                <td data-label="عملیات" class="cell-actions">
                                    <div style="display:inline-flex; gap:6px;">
                                        ...
                                        <form method="post" action="product.php" style="display:inline" onsubmit="return confirm('آیا از حذف این محصول مطمئن هستید؟')">
                                            <input type="hidden" name="action" value="delete">
                                            <input type="hidden" name="id" value="<?php echo (int)$list['id']; ?>">
                                            <button type="submit" class="btn btn-sm btn-soft-danger" title="حذف"><?php echo icon('trash', 'svg-icon'); ?></button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
    ...
    <div class="modal-box" style="max-width:560px;">
        ...
        <form action="product.php" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" name="cetegory_product" class="form-control" list="categoryList" required>
                <datalist id="categoryList">
<?php
try {
    $rxCats = $pdo->query("SELECT remark FROM category ORDER BY id ASC")->fetchAll(PDO::FETCH_COLUMN);
    foreach ($rxCats as $rxCat) {
        echo '<option value="' . htmlspecialchars((string)$rxCat, ENT_QUOTES, 'UTF-8') . '">';
    }
} catch (Throwable $e) {}
?>
                </datalist>
