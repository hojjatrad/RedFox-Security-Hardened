...
function synchronizeXuiPanels(PDO $pdo)
{
    try {
        ...
        error_log('Failed to synchronize x_ui table with marzban_panel: ' . redfox_exception_fingerprint($e));
        throw $e;
    }
}
...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') synchronizeXuiPanels($pdo);
...
if ($action === 'save' && $_SERVER['REQUEST_METHOD'] === 'POST'){
    $selectedPanelCode = trim((string)($_POST['namepanel'] ?? ''));
    $settingsValue = (string)($_POST['settings'] ?? '');
    $allowedPanelCodes = array_column($resultpanel, 'code_panel');
    $decodedSettings = strlen($settingsValue) <= 262144 ? json_decode($settingsValue, true, 32) : null;
    if ($selectedPanelCode === '' || !in_array($selectedPanelCode, $allowedPanelCodes, true)
        || !is_array($decodedSettings) || json_last_error() !== JSON_ERROR_NONE) {
        http_response_code(400);
        exit('پنل یا JSON تنظیمات نامعتبر است');
    }
    $normalizedSettings = json_encode($decodedSettings, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    if (!is_string($normalizedSettings)) { http_response_code(400); exit('JSON نامعتبر است'); }
    $updateStmt = $pdo->prepare("UPDATE x_ui SET setting = :setting WHERE codepanel = :codepanel");
    $updateStmt->execute([':setting' => $normalizedSettings, ':codepanel' => $selectedPanelCode]);
    header('Location: seeting_x_ui.php', true, 303);
    exit;
}

if ($action === 'change' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    $candidatePanelCode = trim((string)($_POST['namepanel'] ?? ''));
    if (!in_array($candidatePanelCode, array_column($resultpanel, 'code_panel'), true)) {
        http_response_code(400); exit('پنل انتخاب‌شده نامعتبر است');
    }
    $selectedPanelCode = $candidatePanelCode;
}
...
<html lang="fa" dir="rtl" data-color="blue" data-theme="dark">
  <head>
    ...
    <title>تنظیمات X-UI — پنل رد فاکس</title>
      ...
      <section class="wrapper">
        ...
        <?php if($action != "change"){ ?>
          <div class="card">
            ...
            <form class="xui-form" role="form" method="POST" action="seeting_x_ui.php">
