...
$SETTING_GROUPS = [
    ...
    'service' => [
        ...
        'fields' => [
            ...
            ['type' => 'number', 'col' => 'timeauto_not_verify','label'=>'تأیید خودکار رسید پس از این زمان (دقیقه) — در صورت تأیید نشدن دستی','placeholder' => '4'],
...
try {
    ...
    error_log('[panel/settings] load failed: ' . redfox_exception_fingerprint($e));
}
            ...
            if ($f['type'] === 'toggle') {
                ...
                if ((string)$cur !== (string)$new) {
                    try {
                        ...
                        error_log('[panel/settings] toggle ' . $col . ' failed: ' . redfox_exception_fingerprint($e));
                    }
                }
            } else {
                if (array_key_exists('f_' . $col, $_POST)) {
                    ...
                    if ((string)$cur !== $new) {
                        try {
                            ...
                            error_log('[panel/settings] text ' . $col . ' failed: ' . redfox_exception_fingerprint($e));
                        }
                    }
                }
            }
...
function redfox_is_toggle_on($cur, $on, $off) {
...
<head>
    ...
    <title>تنظیمات ربات | پنل رد فاکس</title>
                                        ...
                                        <?php if ($f['type'] === 'toggle'): ?>
                                            <label class="switch" title="<?php echo htmlspecialchars($col); ?>">
                                                <input type="checkbox" id="<?php echo $idAttr; ?>" name="<?php echo $idAttr; ?>"
                                                    <?php echo redfox_is_toggle_on($val, $f['on'], $f['off']) ? 'checked' : ''; ?>>
