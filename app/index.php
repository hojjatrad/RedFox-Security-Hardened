...
header("Content-Security-Policy: default-src 'self'; script-src 'self' 'unsafe-inline' https://telegram.org; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; font-src 'self' data:; connect-src 'self'; object-src 'none'; base-uri 'self'; form-action 'self' https:; frame-ancestors 'self' https://web.telegram.org https://*.telegram.org");
...
" onerror="this.onerror=null;var s=document.createElement('script');s.src='<?php echo $sdkFallback; ?>';document.head.appendChild(s);">
    ...
    (function () {
                ...
                fetch(apiUrl + '/clientlog.php', {
                    ...
                    body: JSON.stringify({
                        ...
                        diag: { href: location.href, ua: navigator.userAgent,
                                bootStarted: !!window.__REDFOX_APP_STARTED__ }
                    }),
                    keepalive: true
                }).catch(function () {});
            } catch (e) {  }
        ...
        function showFallback(title, msg) {
            ...
            if (!view) return;
            // Diagnostic messages and stack traces are sent only to the
            // authenticated, redacted client-log endpoint—not rendered publicly.
            view.innerHTML =
                '<div class="empty">' +
        ...
        window.addEventListener('error', function (e) {
            ...
            if (!window.__REDFOX_APP_STARTED__) {
                showFallback('خطا در بارگذاری برنامه', 'برنامه به‌درستی بارگذاری نشد. لطفاً دوباره تلاش کنید.');
            }
        }, true);
...
(function () {
    window.__hardReload = function () {
        ...
        try {
            var preserveKeys = ['red fox.theme.accent', 'red fox.initData'];
            var keep = {};
        ...
        <main id="view" class="view" aria-live="polite">
            <article class="card card-window">
                <header class="card-window-bar">
                    ...
                    <span class="window-url">red fox/loading</span>
                </header>
                <div class="card-body">
