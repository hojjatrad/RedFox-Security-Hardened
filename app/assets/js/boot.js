(function () {
    window.__REDFOX_BOOT_STARTED__ = true;

    function fail(stage, err) {
        var msg = (err && err.message) || String(err || 'unknown');
        var stack = (err && err.stack) || '';
        if (window.__REDFOX__) {
            window.__REDFOX__.postLog('error', '[boot:' + stage + '] ' + msg, 'boot.js', stack);
            window.__REDFOX__.renderError('بارگذاری اپ ناموفق بود', msg, stack);
        } else {
    ...
    var build       = (window.__REDFOX__ && window.__REDFOX__.version) || version;
