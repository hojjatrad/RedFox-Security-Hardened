...
import { setQrImage, releaseQrImage } from '../qr.js';
...
function renderQrPanel(data) {
    ...
    return `
        <div class="qr-panel" id="qr-panel">
            <div class="qr-frame">
                <img id="service-success-qr" alt="QR Code" />
            </div>
    ...
    if ($qrBtn && $qrHost) {
        ...
        $qrBtn.addEventListener('click', () => {
            ...
            if (open) {
                ...
                const img = $qrHost.querySelector('#service-success-qr');
                setQrImage(img, connectionData, 480).catch(() => {
                    if (img) img.alt = 'ساخت QR ناموفق بود';
                });
                $qrBtn.querySelector('span:last-child').textContent = 'بستن QR Code';
                $qrHost.scrollIntoView({ behavior: 'smooth', block: 'center' });
            } else {
                releaseQrImage($qrHost.querySelector('#service-success-qr'));
                $qrHost.innerHTML = '';
