...
function bindAutomaticQrImages(scope) {
    if (!scope) return;
    scope.querySelectorAll('.qr-img[data-qr][data-qr-auto]').forEach((img) => {
        if (img.getAttribute('src') || img.dataset.qrLoading === '1') return;
        const size = Number(img.dataset.qrSize || 280);
        setQrImage(img, img.dataset.qr || '', size).catch(() => {
            img.alt = 'ساخت QR ناموفق بود';
        });
    });
}
    ...
    return '<div class="cfg-list" style="display:flex;flex-direction:column;gap:10px">' + configs.map((c, i) => `
        <div class="cfg-item">
            ...
            <div class="cfg-detail" data-i="${i}" style="display:none;margin:8px 0 12px">
                <div class="qr-block"><img class="qr-img" data-qr="${escapeHtml(c)}" data-qr-size="280" alt="QR" /></div>
    ...
    $scope.querySelectorAll('.cfg-toggle').forEach((btn) => {
        btn.addEventListener('click', () => {
            ...
            if (img && !img.getAttribute('src') && img.dataset.qr) {
                setQrImage(img, img.dataset.qr, Number(img.dataset.qrSize || 280)).catch(() => {
                    img.alt = 'ساخت QR ناموفق بود';
                });
            }
    ...
    view.innerHTML = `
        ...
        <article class="card card-window">
            <header class="card-window-bar">
                ...
                <span class="window-url">red fox/service/${escapeHtml(username)}</span>
            </header>
...
function renderOutput(o) {
    ...
    if (type === 'link') {
        ...
        return `
            ...
            <div class="qr-block">
                <img class="qr-img" data-qr="${escapeHtml(url)}" data-qr-size="280" data-qr-auto="1" alt="QR" loading="lazy" />
            </div>
    ...
    if (type === 'password') {
        ...
        return `
            ...
            ${pw ? `<div class="qr-block">
                <img class="qr-img" data-qr="${escapeHtml(pw)}" data-qr-size="280" data-qr-auto="1" alt="QR" loading="lazy" />
            </div>` : ''}
