...
export async function call(action, { method = null, params = {}, body = null, retry = true } = {}) {
    ...
    const effectiveMethod = method || (action === 'brand_info' ? 'GET' : 'POST');
    const query = effectiveMethod === 'GET' ? { actions: action, ...params } : { actions: action };
    ...
    if (effectiveMethod !== 'GET' && effectiveMethod !== 'HEAD') {
        const payload = { ...params, ...(body || {}), actions: action };
