...
if (isset($pdo) && $pdo instanceof PDO && function_exists('rx_cron_telemetry_start')) rx_cron_telemetry_start($pdo,'sendmessage');
...
try {
    ...
    error_log('[sendmessage.php] ' . redfox_exception_fingerprint($e));
    $rxBcLog('EXCEPTION — ' . redfox_exception_fingerprint($e) . ' @ ' . $e->getFile() . ':' . $e->getLine());
} finally {
    @unlink($lockFile);
}
