...
try {
    ...
    error_log('[cryptocheck] expire pass: ' . redfox_exception_fingerprint($e));
}
...
$rxCryptoLog = static function (string $level, string $msg, array $ctx = []): void {
    if ($level !== 'ERROR') return;
    $safeMsg = preg_replace('/[^A-Za-z0-9 _.-]/', '_', $msg) ?: 'operation failed';
    $safe = [];
    foreach (['order', 'count'] as $key) {
        if (isset($ctx[$key]) && is_scalar($ctx[$key])) {
            $safe[$key] = substr(preg_replace('/[^A-Za-z0-9_.-]/', '_', (string)$ctx[$key]) ?: '', 0, 128);
        }
    }
    error_log('[cryptocheck] ' . substr($safeMsg, 0, 160) . ($safe ? ' ' . json_encode($safe) : ''));
};
...
register_shutdown_function(static function (): void {
    ...
    error_log('[cryptocheck-fatal] type=' . (int)($err['type'] ?? 0)
        ...
        foreach ($stuckList as $stuckRow) {
            ...
            try {
                ...
                $rxCryptoLog('ERROR', 'stuck refund transaction failed', [
                    'order' => $stuckOrderId,
                    'error' => redfox_exception_fingerprint($e),
                ]);
            }
        }
    }
} catch (Throwable $e) {
    $rxCryptoLog('ERROR', 'stuck-refund pass failed', ['error' => redfox_exception_fingerprint($e)]);
}
...
try {
    ...
    if (!empty($retryList)) {
        ...
        foreach ($retryList as $retryRow) {
            ...
            // اجرای دوباره DirectPayment می‌تواند سرویس/اعتبار تکراری بسازد؛ فقط برای تطبیق ادمین علامت‌گذاری می‌کنیم.
            try {
                $pdo->prepare("INSERT INTO payment_effects(order_id,status,source,started_at,updated_at,last_error) VALUES(?,'needs_reconcile','legacy_crypto_retry',?,?,?) ON DUPLICATE KEY UPDATE status=IF(status='completed',status,'needs_reconcile'),updated_at=VALUES(updated_at),last_error=VALUES(last_error)")
                    ->execute([$rOrderId,time(),time(),'paid crypto order has incomplete service effects; automatic retry blocked']);
                $rxCryptoLog('WARN','legacy DirectPayment retry blocked; manual reconciliation required',['order'=>$rOrderId]);
            } catch (Throwable $e) {
                $rxCryptoLog('ERROR','failed to mark payment reconciliation',['order'=>$rOrderId,'error'=>redfox_exception_fingerprint($e)]);
            }
    ...
    $rxCryptoLog('ERROR', 'retry pass failed', ['error' => redfox_exception_fingerprint($e)]);
}
...
try {
    ...
    $rxCryptoLog('ERROR', 'select pending failed', ['error' => redfox_exception_fingerprint($e)]);
    return;
}
...
foreach ($rows as $row) {
    ...
    if (!is_array($result) || empty($result['ok'])) {
        ...
        if (!preg_match('/^[a-z0-9-]{1,64}$/', $reason)) $reason = 'remote-failure';
        try {
            $errStr = json_encode(['reason' => $reason], JSON_UNESCAPED_SLASHES);
            $errUpd = $pdo->prepare(
            ...
            try {
                ...
                if ($rej->rowCount() > 0) {
                    ...
                    try {
                        ...
                        $rxCryptoLog('ERROR', 'admin notify on reject failed', ['order' => $orderId, 'error' => redfox_exception_fingerprint($e)]);
                    }
                }
            } catch (Throwable $e) {  }
        }
    ...
    try {
        ...
        $pdo->prepare("UPDATE Payment_report SET at_updated=?, crypto_sender_address=COALESCE(NULLIF(crypto_sender_address,''),?) WHERE id_order=? AND payment_Status<>'paid'")
            ->execute([date('Y/m/d H:i:s'),$verifiedSender!==''?$verifiedSender:null,$orderId]);
        if (function_exists('crypto_record_verified_hash')) crypto_record_verified_hash($orderId,'auto_cron');
        $confirm=payment_confirm_paid($orderId,'',['method'=>'تایید خودکار رمزارز','expected_method'=>'arze digital offline','thread_id'=>$paymentreports]);
        if(empty($confirm['ok'])){$rxCryptoLog('WARN','idempotent payment confirm deferred',['order'=>$orderId,'result'=>$confirm]);continue;}
        $rxCryptoLog('OK','payment effects completed',['order'=>$orderId,'sender'=>$verifiedSender]);
    } catch (Throwable $e) {
        $rxCryptoLog('ERROR','idempotent payment confirmation failed',['order'=>$orderId,'error'=>redfox_exception_fingerprint($e)]);
        continue;
    }
    ...
    try {
        ...
        $rxCryptoLog('ERROR', 'post-paid handling threw', ['order' => $orderId, 'error' => redfox_exception_fingerprint($e)]);
    }
}
