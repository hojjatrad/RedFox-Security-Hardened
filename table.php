<?php
if (!defined('REDFOX_SCHEMA_MIGRATION_MODE')) define('REDFOX_SCHEMA_MIGRATION_MODE', true);
if (PHP_SAPI !== 'cli' && !(defined('REDFOX_INSTALLER_SCHEMA_CALL') && REDFOX_INSTALLER_SCHEMA_CALL === true)) { http_response_code(404); exit; }
...
if (function_exists('putenv')) @putenv('TZ=Asia/Tehran');
...
try {
    ...
    }
    {
        ...
        // Red Fox: ذخیره‌ی نام تلگرامی (first_name) برای نمایش در پنل
        addFieldToTable($tableName, 'tg_name', '', 'VARCHAR(300)');
        // Red Fox: متون قابل ویرایش پیام خوش‌آمدگویی و راهنمای نصب
        addFieldToTable($tableName, 'admin_welcome_text', '', 'TEXT');
        addFieldToTable($tableName, 'admin_setup_text', '', 'TEXT');
        // Red Fox: رمز عبور پورتال نماینده
        addFieldToTable($tableName, 'panel_password', '', 'VARCHAR(200)');
    ...
    error_log('[panels] ' . redfox_exception_fingerprint($e));
}
...
try {
    ...
    if (!$tableExists) {
        $stmt = $pdo->prepare("CREATE TABLE $tableName (
        id_admin varchar(191) PRIMARY KEY NOT NULL,
...
try {
    ...
    if (!$table_exists) {
        ...
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_unicode_ci");
        if (!$result) {
            error_log("[table.php] table marzban_panel: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
    } else {
        ...
        addFieldToTable("marzban_panel", "provisioning_status", "incomplete", "VARCHAR(20)");
        addFieldToTable("marzban_panel", "provisioning_error", null, "VARCHAR(1000)");
        addFieldToTable("marzban_panel", "reference_username", null, "VARCHAR(191)");
    }
...
try {
    ...
    if (!$table_exists) {
        ...
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_unicode_ci");
        if (!$result) {
            error_log("[table.php] table product: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
    } else {
...
try {
    ...
    if (!$table_exists) {
        ...
        id_invoice varchar(191) PRIMARY KEY,
        ...
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_unicode_ci");
        if (!$result) {
            error_log("[table.php] table invoice: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
    } else {
...
try {
    ...
    if (!$table_exists) {
        ...
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
        if (!$result) {
            error_log("[table.php] table Payment_report: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
    } else {
...
try {
    ...
    if (!$table_exists) {
        ...
        ");
        if (!$result) {
            error_log("[table.php] table Discount: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
    } else {
...
try {
    ...
    if (!$table_exists) {
        ...
        id_user varchar(200) NULL)");
        if (!$result) {
            error_log("[table.php] table Giftcodeconsumed: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
    }
} catch (Exception $e) {
    error_log('[panels] ' . redfox_exception_fingerprint($e));
...
try {
    ...
    if ($table_exists) {
        try {
            ...
            error_log('[table.php textbot pk] ' . redfox_exception_fingerprint($rxTbPkErr));
        }
    }
    $text_roll = "
...
🔝لزومی به ارسال رسید نیست، اما در صورتی که بعد از گذشت مدتی واریز شما تایید نشد، عکس رسید خود را ارسال کنید.";
    ...
    if (!$table_exists) {
        ...
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_unicode_ci");
        if (!$result) {
            error_log("[table.php] table textbot: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
...
try {
    ...
    if (!$table_exists) {
        ...
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_unicode_ci");
        if (!$result) {
            error_log("[table.php] table PaySetting: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
...
try {
    ...
    if (!$table_exists) {
        ...
        if (!$result) {
            error_log("[table.php] table DiscountSell: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
    } else {
...
try {
    ...
    error_log('[discount-migrate] ' . redfox_exception_fingerprint($e));
}
...
try {
    ...
    if (!$table_exists) {
        ...
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_unicode_ci");
        if (!$result) {
            error_log("[table.php] table affiliates: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
...
try {
    ...
    if ($table_exists) {
        try {
            ...
            error_log('[table.php shopSetting pk] ' . redfox_exception_fingerprint($rxShopPkErr));
        }
    }
    ...
    if (!$table_exists) {
        ...
        Namevalue varchar(191) PRIMARY KEY NOT NULL,
        value TEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL)
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_unicode_ci");
        if (!$result) {
            error_log("[table.php] table shopSetting: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
...
try {
    ...
    if (!$table_exists) {
        ...
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_unicode_ci");
        if (!$result) {
            error_log("[table.php] table cancel_service: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
    }
} catch (Exception $e) {
    error_log('[panels] ' . redfox_exception_fingerprint($e));
...
try {
    ...
    if (!$table_exists) {
        ...
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_unicode_ci");
        if (!$result) {
            error_log("[table.php] table service_other: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
    } else {
...
try {
    ...
    if (!$table_exists) {
        ...
        cardnumber varchar(191) PRIMARY KEY,
        namecard  varchar(1000)  NOT NULL)
        CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");
        if (!$result) {
            error_log("[table.php] table x_ui: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
    }
    ...
    if ($columnInfo) {
        ...
        if (empty($currentCollation) || stripos($currentCollation, 'utf8mb4') === false) {
            ...
            $connect->query("ALTER TABLE card_number MODIFY cardnumber varchar(191) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci PRIMARY KEY");
...
try {
    ...
    if (!$table_exists) {
        ...
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
        if (!$result) {
            error_log("[table.php] table Requestagent: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
    } else {
...
try {
    ...
    if (!$table_exists) {
        ...
        report varchar(191) PRIMARY KEY NOT NULL,
...
try {
    ...
    if (!$table_exists) {
        ...
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_unicode_ci");
        if (!$result) {
            error_log("[table.php] table manualsell: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
    }
} catch (Exception $e) {
    error_log('[panels] ' . redfox_exception_fingerprint($e));
...
try {
    ...
    if (!$table_exists) {
        ...
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_unicode_ci");
        if (!$result) {
            error_log("[table.php] table wheel_list: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
    }
} catch (Exception $e) {
    error_log('[panels] ' . redfox_exception_fingerprint($e));
...
try {
    ...
    if (!$table_exists) {
        ...
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_unicode_ci");
        if (!$result) {
            error_log("[table.php] table botsaz: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
    } else {
...
try {
    ...
    if (!$table_exists) {
        ...
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_unicode_ci");
        if (!$result) {
            error_log("[table.php] table app: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
    }
} catch (Exception $e) {
    error_log('[panels] ' . redfox_exception_fingerprint($e));
...
try {
    ...
    if (!$table_exists) {
        ...
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_unicode_ci");
        if (!$result) {
            error_log("[table.php] table logs_api: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
    }
} catch (Exception $e) {
    error_log('[panels] ' . redfox_exception_fingerprint($e));
...
try {
    ...
    if (!$table_exists) {
        ...
        if (!$result) {
            error_log("[table.php] table category: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
    }
} catch (Exception $e) {
    error_log('[panels] ' . redfox_exception_fingerprint($e));
...
try {
    ...
    error_log('[table.php channels charset] ' . redfox_exception_fingerprint($e));
}



try {
    addFieldToTable("user", "token_expires_at", "0", "BIGINT UNSIGNED NOT NULL");
    addFieldToTable("setting", "webhook_secret_token", "", "VARCHAR(64)");
    addFieldToTable("setting", "webhook_setup_status", "pending", "VARCHAR(16) NOT NULL DEFAULT 'pending'");
    addFieldToTable("setting", "webhook_last_error_code", "", "VARCHAR(64) NOT NULL DEFAULT ''");
    addFieldToTable("setting", "webhook_last_attempt_at", "0", "BIGINT UNSIGNED NOT NULL DEFAULT 0");
    addFieldToTable("setting", "integration_secret_token", "", "VARCHAR(64)");
    $rxIntegrationSecret = (string)($pdo->query("SELECT integration_secret_token FROM setting LIMIT 1")->fetchColumn() ?: '');
    if ($rxIntegrationSecret === '') {
        $rxIntegrationSecret = bin2hex(random_bytes(32));
        $pdo->prepare("UPDATE setting SET integration_secret_token = ?")->execute([$rxIntegrationSecret]);
    }
    ...
    error_log('Failed to read/persist webhook_secret_token: ' . redfox_exception_fingerprint($rxSecretErr));
    $rxBotWebhookSecret = '';
}
// Webhook registration is intentionally not performed while creating/upgrading
// tables. The installer and panel/bot_settings.php own the validated, retriable
// setup and persist only an allow-listed recovery status/error code.
...
try {
    $connect->query("CREATE TABLE IF NOT EXISTS nm_config_stock (
        ...
        KEY idx_nm_stock_lookup (status, codepanel(63), codeproduct(63), tier),
    ...
    $connect->query("CREATE TABLE IF NOT EXISTS nm_stock_shelves (
        ...
        UNIQUE KEY uq_nm_stock_shelf_name_panel (name(100), source_codepanel(63)),
        KEY idx_nm_stock_shelf_lookup (status, source_codepanel(63), codeproduct(63))
    ) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");
    ...
    $connect->query("CREATE TABLE IF NOT EXISTS nm_stock_product_map (
        ...
        created_at INT UNSIGNED NOT NULL DEFAULT 0,
        updated_at INT UNSIGNED NULL,
        UNIQUE KEY uq_nm_stock_product_map (source_codepanel(63), stock_codepanel(63), codeproduct(63)),
        KEY idx_nm_stock_product_lookup (status, source_codepanel(63), codeproduct(63)),
        KEY idx_nm_stock_product_category (status, source_codepanel(63), category(63))
    ) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");
    ...
    redfox_log_exception($e, 'table.national_inventory');
}
...
try {
    ...
    if (!$exists) {
        ...
        if (!$created) {
            error_log("[table.php] crypto_wallets: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
        $seedCurrencies = [
            ['TRX',         'TRON', '', 'ترون (TRX)'],
            ['TON',         'TON',  '', 'تون (TON)'],
    ...
    try {
        ...
        if ($haveSrcIdx && $haveSrcIdx->num_rows === 0) {
            @$connect->query("ALTER TABLE Payment_report ADD INDEX idx_pr_source_user (source, id_user(100))");
        }
    } catch (Throwable $e) {  }
    ...
    try {
        ...
        if ($haveHashIdx && $haveHashIdx->num_rows === 0) {
            @$connect->query("ALTER TABLE Payment_report ADD UNIQUE KEY uniq_pr_crypto_hash (crypto_tx_hash(191))");
        }
    } catch (Throwable $e) {
        error_log('[table.php] could not add unique key on crypto_tx_hash: ' . redfox_exception_fingerprint($e));
    }
    ...
    try {
        ...
        if ($haveSenderIdx && $haveSenderIdx->num_rows === 0) {
            $connect->query("ALTER TABLE Payment_report ADD INDEX idx_pr_crypto_sender (crypto_currency, crypto_sender_address(100))");
        }
    } catch (Throwable $e) {  }
    ...
    try {
        $connect->query("CREATE TABLE IF NOT EXISTS crypto_verified_hashes (
            ...
            UNIQUE KEY uniq_cvh_hash (tx_hash(191)),
        ...
        error_log('[table.php] crypto_verified_hashes create: ' . redfox_exception_fingerprint($e));
    }
    ...
    try {
        $connect->query("CREATE TABLE IF NOT EXISTS crypto_sender_locks (
            ...
            UNIQUE KEY uniq_sender_currency (sender_address(100), currency),
            INDEX idx_sender_user (telegram_user_id),
            INDEX idx_sender_first_seen (first_seen_at)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    } catch (Throwable $e) {
        error_log('[table.php] crypto_sender_locks create: ' . redfox_exception_fingerprint($e));
    }
    ...
    redfox_log_exception($e, 'table.cryptocheck');
}
...
try {
    ...
    if (!$exists) {
        ...
        if (!$created) {
            error_log("[table.php] premium_emojis: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
    ...
    try {
        ...
        error_log("[table.php] premium_emojis index migration: " . redfox_exception_fingerprint($idxErr));
    }


    echo "premium emoji schema OK ✅";
} catch (Exception $e) {
    redfox_log_exception($e, 'table.premium_emojis');
}
...
try {
    ...
    redfox_log_exception($e, 'table.setting_keyboard_columns');
}
...
try {
    ...
    redfox_log_exception($rxSeedErr, 'table.keyboard_defaults_seed');
}

// ── Red Fox: پشتیبانی هوش مصنوعی (فاز ۱) ──────────────────────────────
// ستون تنظیمات AI روی setting (JSON: status, api_url, api_key, model, system_prompt, escalate_keywords, max_history)
addFieldToTable("setting", "ai_support", "", "TEXT");

try {
    $result = $connect->query("SHOW TABLES LIKE 'ai_support_log'");
    $table_exists = ($result && $result->num_rows > 0);
    if (!$table_exists) {
        $result = $connect->query("CREATE TABLE ai_support_log (
        ...
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
        if (!$result) {
            error_log("[table.php] table ai_support_log: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
    }
} catch (Exception $e) {
    error_log('[table.php] ai_support_log: ' . redfox_exception_fingerprint($e));
}

// ── Red Fox: پایگاه دانش هوش مصنوعی (فاز ۳) ───────────────────────────
// منبع یادگیری: پاسخ‌های ادمین (admin)، ورودی دستی (manual)، فایل آپلودی (file)
try {
    $result = $connect->query("SHOW TABLES LIKE 'ai_knowledge'");
    $table_exists = ($result && $result->num_rows > 0);
    if (!$table_exists) {
        $result = $connect->query("CREATE TABLE ai_knowledge (
        id INT(11) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
        question TEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NULL,
        answer MEDIUMTEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NULL,
        ...
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_unicode_ci");
        if (!$result) {
            error_log("[table.php] table ai_knowledge: " . 'mysql_errno_' . (int)mysqli_errno($connect));
        }
    }
} catch (Exception $e) {
    error_log('[table.php] ai_knowledge: ' . redfox_exception_fingerprint($e));
}

// ── Red Fox: کارت‌های بانکی اختصاصی نماینده (فاز ۲) ────────────────────
try {
    $result = $connect->query("SHOW TABLES LIKE 'reseller_cards'");
    $table_exists = ($result && $result->num_rows > 0);
    if (!$table_exists) {
        $result = $connect->query("CREATE TABLE reseller_cards (
        id INT(11) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
        ...
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_unicode_ci");
        if (!$result) { error_log("[table.php] reseller_cards: " . 'mysql_errno_' . (int)mysqli_errno($connect)); }
    }
} catch (Exception $e) {
    error_log('[table.php] reseller_cards: ' . redfox_exception_fingerprint($e));
}


// ── Red Fox: اشتراک هوش مصنوعی برای ربات‌های نماینده ───────────────────
// ادمین می‌تواند برای هر نماینده قابلیت AI را فعال/غیرفعال کند،
// تاریخ انقضا تعیین یا تمدید کند، و قیمت تعیین کند.
try {
    $result = $connect->query("SHOW TABLES LIKE 'reseller_ai_feature'");
    $table_exists = ($result && $result->num_rows > 0);
    if (!$table_exists) {
        $result = $connect->query("CREATE TABLE reseller_ai_feature (
        id INT(11) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
        reseller_id VARCHAR(200) NOT NULL,
        bot_token VARCHAR(500) NOT NULL,
        bot_username VARCHAR(200) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NULL,
        status ENUM('active','expired','pending','disabled') NOT NULL DEFAULT 'disabled',
        ...
        UNIQUE KEY uniq_rxf_bot_token (bot_token(191)))
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
        if (!$result) { error_log("[table.php] reseller_ai_feature: " . 'mysql_errno_' . (int)mysqli_errno($connect)); }
    } else {
        rxSafeAddColumn($connect, "reseller_ai_feature", "bot_username", "VARCHAR(200) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NULL");
        rxSafeAddColumn($connect, "reseller_ai_feature", "days", "INT(11) NOT NULL DEFAULT 30");
        rxSafeAddColumn($connect, "reseller_ai_feature", "notes", "TEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NULL");
    }
} catch (Exception $e) {
    error_log('[table.php] reseller_ai_feature: ' . redfox_exception_fingerprint($e));
}

// ستون‌های تنظیمات قیمت/مدت پیش‌فرض روی setting
addFieldToTable("setting", "reseller_ai_price", "0", "VARCHAR(100)");
addFieldToTable("setting", "reseller_ai_days", "30", "VARCHAR(20)");
// ستون bottype روی ai_support_log برای تفکیک لاگ‌های ربات نماینده
addFieldToTable("ai_support_log", "bottype", "main", "VARCHAR(500)");

// ── Red Fox: کش داده‌ی زنده‌ی کاربران از پنل (برای سینک گروهی) ──────────
try {
    $connect->query("CREATE TABLE IF NOT EXISTS rx_user_cache (
        id INT(11) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
        user_id VARCHAR(200) NOT NULL,
        invoice_id VARCHAR(200) NOT NULL,
        username VARCHAR(300) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NULL,
        panel_name VARCHAR(300) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NULL,
        ...
        sub_url TEXT NULL,
        synced_at VARCHAR(50) NULL,
        UNIQUE KEY uniq_rxc_invoice (invoice_id(191)))
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
} catch (Exception $e) {
    error_log('[table.php] rx_user_cache: ' . redfox_exception_fingerprint($e));
}

// ── Red Fox: انقضای ربات نماینده + شارژ ──────────────────────────────
addFieldToTable("botsaz", "bot_expires_at", null, "VARCHAR(50)");
addFieldToTable("botsaz", "bot_status", "active", "VARCHAR(30)");
addFieldToTable("botsaz", "bot_charge_amount", "0", "VARCHAR(100)");

// ── Red Fox: دسترسی‌ها و سلسله‌مراتب نماینده ─────────────────────────
addFieldToTable("user", "reseller_perms", "{}", "TEXT");
addFieldToTable("user", "reseller_role", "agent", "VARCHAR(20) NOT NULL");
addFieldToTable("user", "reseller_parent_id", null, "VARCHAR(64) NULL");
addFieldToTable("user", "reseller_portal_status", "active", "VARCHAR(20) NOT NULL");
try {
    $connect->query("CREATE TABLE IF NOT EXISTS reseller_audit_log (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,reseller_id VARCHAR(64) NOT NULL,actor_role VARCHAR(20) NOT NULL,action VARCHAR(100) NOT NULL,entity VARCHAR(100) NULL,details TEXT NULL,ip VARCHAR(100) NULL,created_at BIGINT UNSIGNED NOT NULL,KEY idx_ral_owner_time(reseller_id,created_at)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    $connect->query("CREATE TABLE IF NOT EXISTS reseller_categories (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,reseller_id VARCHAR(64) NOT NULL,name VARCHAR(191) NOT NULL,created_at BIGINT UNSIGNED NOT NULL,UNIQUE KEY uq_reseller_category(reseller_id,name(120))) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    $connect->query("CREATE TABLE IF NOT EXISTS reseller_wallet_ledger (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,from_user_id VARCHAR(64) NOT NULL,to_user_id VARCHAR(64) NOT NULL,amount BIGINT UNSIGNED NOT NULL,reason VARCHAR(100) NOT NULL,actor_reseller_id VARCHAR(64) NOT NULL,created_at BIGINT UNSIGNED NOT NULL,KEY idx_rwl_actor_time(actor_reseller_id,created_at),KEY idx_rwl_users(from_user_id,to_user_id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    $connect->query("CREATE TABLE IF NOT EXISTS reseller_service_operations (operation_id CHAR(32) PRIMARY KEY,idempotency_key VARCHAR(64) NOT NULL UNIQUE,actor_reseller_id VARCHAR(64) NOT NULL,owner_reseller_id VARCHAR(64) NOT NULL,invoice_id VARCHAR(200) NOT NULL,action VARCHAR(40) NOT NULL,amount INT UNSIGNED NOT NULL DEFAULT 0,price BIGINT UNSIGNED NOT NULL DEFAULT 0,status VARCHAR(30) NOT NULL,result MEDIUMTEXT NULL,created_at BIGINT UNSIGNED NOT NULL,updated_at BIGINT UNSIGNED NOT NULL,KEY idx_rso_owner_time(owner_reseller_id,created_at),KEY idx_rso_invoice(invoice_id,created_at),KEY idx_rso_status(status,updated_at)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    $connect->query("CREATE TABLE IF NOT EXISTS schema_migrations (version VARCHAR(191) PRIMARY KEY,checksum CHAR(64) NOT NULL,executed_at BIGINT UNSIGNED NOT NULL,execution_ms INT UNSIGNED NOT NULL DEFAULT 0) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    $connect->query("CREATE TABLE IF NOT EXISTS reseller_broadcasts (broadcast_id CHAR(32) PRIMARY KEY,actor_reseller_id VARCHAR(64) NOT NULL,title VARCHAR(191) NOT NULL DEFAULT '',message TEXT NOT NULL,kind VARCHAR(20) NOT NULL DEFAULT 'broadcast',status VARCHAR(20) NOT NULL DEFAULT 'queued',total_count INT UNSIGNED NOT NULL DEFAULT 0,sent_count INT UNSIGNED NOT NULL DEFAULT 0,failed_count INT UNSIGNED NOT NULL DEFAULT 0,created_at BIGINT UNSIGNED NOT NULL,completed_at BIGINT UNSIGNED NULL,KEY idx_rb_actor_time(actor_reseller_id,created_at),KEY idx_rb_status(status,created_at)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    $connect->query("CREATE TABLE IF NOT EXISTS reseller_message_queue (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,broadcast_id CHAR(32) NOT NULL,recipient_user_id VARCHAR(64) NOT NULL,bot_owner_id VARCHAR(64) NULL,status VARCHAR(20) NOT NULL DEFAULT 'queued',attempts TINYINT UNSIGNED NOT NULL DEFAULT 0,next_attempt_at BIGINT UNSIGNED NOT NULL DEFAULT 0,claim_token CHAR(32) NULL,claimed_at BIGINT UNSIGNED NULL,last_error VARCHAR(1000) NULL,sent_at BIGINT UNSIGNED NULL,UNIQUE KEY uq_rmq_broadcast_user(broadcast_id,recipient_user_id),KEY idx_rmq_dispatch(status,next_attempt_at,id),KEY idx_rmq_claim(claim_token)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    $connect->query("CREATE TABLE IF NOT EXISTS reseller_api_keys (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,reseller_id VARCHAR(64) NOT NULL,name VARCHAR(100) NOT NULL,key_prefix VARCHAR(20) NOT NULL,key_hash CHAR(64) NOT NULL,permissions TEXT NOT NULL,expires_at BIGINT UNSIGNED NOT NULL,last_used_at BIGINT UNSIGNED NULL,revoked_at BIGINT UNSIGNED NULL,created_at BIGINT UNSIGNED NOT NULL,UNIQUE KEY uq_rak_hash(key_hash),KEY idx_rak_owner(reseller_id,revoked_at)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    $connect->query("CREATE TABLE IF NOT EXISTS payment_effects (order_id VARCHAR(191) PRIMARY KEY,status VARCHAR(30) NOT NULL,source VARCHAR(100) NOT NULL DEFAULT '',started_at BIGINT UNSIGNED NOT NULL,updated_at BIGINT UNSIGNED NOT NULL,last_error VARCHAR(2000) NULL,KEY idx_pe_status_time(status,updated_at)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    $connect->query("CREATE TABLE IF NOT EXISTS cron_job_runs (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,job_name VARCHAR(100) NOT NULL,run_id CHAR(32) NOT NULL,status VARCHAR(20) NOT NULL,started_at BIGINT UNSIGNED NOT NULL,finished_at BIGINT UNSIGNED NULL,processed_count INT UNSIGNED NOT NULL DEFAULT 0,error_summary VARCHAR(2000) NULL,host_name VARCHAR(191) NULL,UNIQUE KEY uq_cjr_run(run_id),KEY idx_cjr_job_time(job_name,started_at),KEY idx_cjr_status(status,started_at)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    addFieldToTable("user", "portal_2fa_enabled", "0", "TINYINT(1) NOT NULL");
    $connect->query("CREATE TABLE IF NOT EXISTS reseller_sessions (session_hash CHAR(64) PRIMARY KEY,reseller_id VARCHAR(64) NOT NULL,ip VARCHAR(100) NULL,ua_hash CHAR(64) NOT NULL,ua_label VARCHAR(255) NULL,created_at BIGINT UNSIGNED NOT NULL,last_seen_at BIGINT UNSIGNED NOT NULL,expires_at BIGINT UNSIGNED NOT NULL,revoked_at BIGINT UNSIGNED NULL,KEY idx_rs_owner_active(reseller_id,revoked_at,expires_at)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    $connect->query("CREATE TABLE IF NOT EXISTS update_history (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,version VARCHAR(100) NOT NULL,previous_version VARCHAR(100) NULL,package_sha256 CHAR(64) NOT NULL,status VARCHAR(30) NOT NULL,actor VARCHAR(191) NULL,details TEXT NULL,key_id VARCHAR(100) NULL,channel VARCHAR(30) NULL,backup_path VARCHAR(1000) NULL,created_at BIGINT UNSIGNED NOT NULL,KEY idx_uh_version_time(version,created_at),KEY idx_uh_status(status,created_at)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    $connect->query("CREATE TABLE IF NOT EXISTS reseller_recovery_codes (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,reseller_id VARCHAR(64) NOT NULL,code_hash VARCHAR(255) NOT NULL,created_at BIGINT UNSIGNED NOT NULL,used_at BIGINT UNSIGNED NULL,KEY idx_rrc_owner_unused(reseller_id,used_at)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    $connect->query("CREATE TABLE IF NOT EXISTS panel_test_accounts (panel_code VARCHAR(191) PRIMARY KEY,test_username VARCHAR(191) NOT NULL,updated_at BIGINT UNSIGNED NOT NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    $connect->query("CREATE TABLE IF NOT EXISTS certification_runs (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,status VARCHAR(20) NOT NULL,score INT UNSIGNED NOT NULL,report_json MEDIUMTEXT NOT NULL,created_by VARCHAR(191) NULL,created_at BIGINT UNSIGNED NOT NULL,KEY idx_cert_time(created_at),KEY idx_cert_status(status,created_at)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    $connect->query("CREATE TABLE IF NOT EXISTS update_sources (id TINYINT UNSIGNED PRIMARY KEY,github_repo VARCHAR(191) NULL,github_token VARCHAR(500) NULL,asset_pattern VARCHAR(191) NOT NULL DEFAULT 'RedFox*.zip',allow_unsigned_local TINYINT NOT NULL DEFAULT 0,auto_check TINYINT NOT NULL DEFAULT 1,last_check_at BIGINT UNSIGNED NULL,last_version VARCHAR(100) NULL,last_url VARCHAR(1000) NULL,last_source VARCHAR(30) NULL,last_file VARCHAR(500) NULL,last_error VARCHAR(1000) NULL,updated_at BIGINT UNSIGNED NOT NULL DEFAULT 0) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    $connect->query("INSERT IGNORE INTO update_sources(id) VALUES(1)");
    $connect->query("CREATE TABLE IF NOT EXISTS update_jobs (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,job_id CHAR(32) NOT NULL,version VARCHAR(100) NOT NULL,source VARCHAR(30) NOT NULL,signed_package TINYINT NOT NULL DEFAULT 0,status VARCHAR(30) NOT NULL,progress_percent TINYINT UNSIGNED NOT NULL DEFAULT 0,progress_stage VARCHAR(100) NULL,processed_files INT UNSIGNED NOT NULL DEFAULT 0,total_action_files INT UNSIGNED NOT NULL DEFAULT 0,total_files INT UNSIGNED NOT NULL DEFAULT 0,changed_files INT UNSIGNED NOT NULL DEFAULT 0,added_files INT UNSIGNED NOT NULL DEFAULT 0,skipped_files INT UNSIGNED NOT NULL DEFAULT 0,file_backup_path VARCHAR(1000) NULL,database_backup_path VARCHAR(1000) NULL,error_message VARCHAR(2000) NULL,actor VARCHAR(191) NULL,created_at BIGINT UNSIGNED NOT NULL,started_at BIGINT UNSIGNED NULL,finished_at BIGINT UNSIGNED NULL,UNIQUE KEY uq_uj_job(job_id),KEY idx_uj_status_time(status,created_at)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    $connect->query("CREATE TABLE IF NOT EXISTS reseller_ticket_attachments (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,tracking VARCHAR(100) NOT NULL,reseller_id VARCHAR(64) NOT NULL,uploader_type VARCHAR(20) NOT NULL,uploader_id VARCHAR(64) NOT NULL,storage_name VARCHAR(500) NOT NULL,original_name VARCHAR(191) NOT NULL,mime_type VARCHAR(100) NOT NULL,file_size BIGINT UNSIGNED NOT NULL,sha256 CHAR(64) NOT NULL,created_at BIGINT UNSIGNED NOT NULL,KEY idx_rta_ticket(tracking,reseller_id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    $connect->query("CREATE TABLE IF NOT EXISTS reseller_payout_accounts (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,reseller_id VARCHAR(64) NOT NULL,holder_name VARCHAR(191) NOT NULL,iban_cipher TEXT NOT NULL,iban_last4 CHAR(4) NOT NULL,status VARCHAR(20) NOT NULL DEFAULT 'pending',is_active TINYINT NOT NULL DEFAULT 1,verified_by VARCHAR(191) NULL,verified_at BIGINT UNSIGNED NULL,created_at BIGINT UNSIGNED NOT NULL,KEY idx_rpa_owner(reseller_id,is_active,status)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    $connect->query("CREATE TABLE IF NOT EXISTS reseller_settlement_settings (id TINYINT UNSIGNED PRIMARY KEY,enabled TINYINT NOT NULL DEFAULT 0,min_amount BIGINT UNSIGNED NOT NULL DEFAULT 100000,fee_percent DECIMAL(5,2) NOT NULL DEFAULT 0,hold_hours INT UNSIGNED NOT NULL DEFAULT 24,updated_at BIGINT UNSIGNED NOT NULL DEFAULT 0) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    $connect->query("INSERT IGNORE INTO reseller_settlement_settings(id) VALUES(1)");
    $connect->query("CREATE TABLE IF NOT EXISTS reseller_settlements (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,request_id CHAR(32) NOT NULL,reseller_id VARCHAR(64) NOT NULL,payout_account_id BIGINT UNSIGNED NOT NULL,amount BIGINT UNSIGNED NOT NULL,fee BIGINT UNSIGNED NOT NULL DEFAULT 0,net_amount BIGINT UNSIGNED NOT NULL,status VARCHAR(20) NOT NULL,idempotency_key VARCHAR(100) NOT NULL,requested_at BIGINT UNSIGNED NOT NULL,reviewed_at BIGINT UNSIGNED NULL,reviewed_by VARCHAR(191) NULL,paid_at BIGINT UNSIGNED NULL,paid_by VARCHAR(191) NULL,payment_reference VARCHAR(191) NULL,admin_note VARCHAR(1000) NULL,UNIQUE KEY uq_rs_request(request_id),UNIQUE KEY uq_rs_idem(idempotency_key),KEY idx_rs_owner_status(reseller_id,status,requested_at)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    $connect->query("CREATE TABLE IF NOT EXISTS reseller_product_categories (reseller_id VARCHAR(64) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,product_code VARCHAR(200) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,category_id BIGINT UNSIGNED NOT NULL,created_at BIGINT UNSIGNED NOT NULL,PRIMARY KEY(reseller_id,product_code),KEY idx_rpc_category(reseller_id,category_id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    $connect->query("CREATE TABLE IF NOT EXISTS reseller_api_nonces (api_key_id BIGINT UNSIGNED NOT NULL,nonce VARCHAR(100) NOT NULL,used_at BIGINT UNSIGNED NOT NULL,PRIMARY KEY(api_key_id,nonce),KEY idx_ran_time(used_at)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    $connect->query("CREATE TABLE IF NOT EXISTS reseller_api_idempotency (api_key_id BIGINT UNSIGNED NOT NULL,idempotency_key VARCHAR(100) NOT NULL,endpoint VARCHAR(100) NOT NULL,request_hash CHAR(64) NOT NULL,status_code INT NULL,response_body MEDIUMTEXT NULL,created_at BIGINT UNSIGNED NOT NULL,completed_at BIGINT UNSIGNED NULL,PRIMARY KEY(api_key_id,idempotency_key),KEY idx_rai_time(created_at)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    rxSafeAddColumn($connect,'reseller_categories','slug','VARCHAR(191) NULL');rxSafeAddColumn($connect,'reseller_categories','sort_order','INT NOT NULL DEFAULT 0');rxSafeAddColumn($connect,'reseller_categories','status',"VARCHAR(20) NOT NULL DEFAULT 'active'");rxSafeAddColumn($connect,'reseller_categories','updated_at','BIGINT UNSIGNED NULL');rxSafeAddColumn($connect,'reseller_api_keys','ip_allowlist','TEXT NULL');
} catch (Throwable $e) { error_log('[table.php reseller portal] '.redfox_exception_fingerprint($e)); }

// ── Red Fox: لاگ ممیزی + احراز هویت دو مرحله‌ای ──────────────────────
try {
    $result = $connect->query("SHOW TABLES LIKE 'audit_log'");
    if (!$result || $result->num_rows == 0) {
        $connect->query("CREATE TABLE audit_log (
        id INT(11) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
        admin_user VARCHAR(200) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NULL,
        action VARCHAR(200) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
        entity VARCHAR(200) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NULL,
    ...
    error_log('[table.php] audit_log: ' . redfox_exception_fingerprint($e));
}

addFieldToTable("setting", "twofa_status", "off", "VARCHAR(10)");
addFieldToTable("setting", "twofa_admin_id", "0", "VARCHAR(200)");
addFieldToTable("admin", "twofa_secret", "", "VARCHAR(200)");

// ── Red Fox: کلید REST API + تمدید خودکار ────────────────────────────
addFieldToTable("setting", "rest_api_key", "", "VARCHAR(200)");
addFieldToTable("user", "auto_renew", "0", "TINYINT(1)");

// ════════════════════════════════════════════════════════════════════
//  Red Fox — قابلیت‌های پیشرفته (Flash Sale, Daily Reward, Win-back, ...)
// ════════════════════════════════════════════════════════════════════

// ── ۱) کمپین فروش ویژه (Flash Sale) ──────────────────────────────────
try {
    $result = $connect->query("SHOW TABLES LIKE 'flash_campaigns'");
    if (!$result || $result->num_rows == 0) {
        $connect->query("CREATE TABLE flash_campaigns (
        id INT(11) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
        title VARCHAR(500) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
        discount_percent INT(3) NOT NULL DEFAULT 0,
        product_filter TEXT NULL,
...
} catch (Exception $e) { error_log('[table.php] flash_campaigns: ' . redfox_exception_fingerprint($e)); }

// ── ۲) سیستم پاداش روزانه (Daily Reward) ────────────────────────────
try {
    $result = $connect->query("SHOW TABLES LIKE 'daily_rewards'");
    if (!$result || $result->num_rows == 0) {
        $connect->query("CREATE TABLE daily_rewards (
        id INT(11) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
        user_id VARCHAR(200) NOT NULL,
        ...
        UNIQUE KEY uniq_dr_user (user_id(191)))
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    }
} catch (Exception $e) { error_log('[table.php] daily_rewards: ' . redfox_exception_fingerprint($e)); }
addFieldToTable("setting", "daily_reward_status", "off", "VARCHAR(10)");
addFieldToTable("setting", "daily_reward_amount", "1000", "VARCHAR(50)");
addFieldToTable("setting", "daily_reward_streak_bonus", "500", "VARCHAR(50)");
addFieldToTable("setting", "daily_reward_max_streak", "7", "VARCHAR(10)");

// ── ۴) امنیت اکانت — محدودیت دستگاه/IP ──────────────────────────────
addFieldToTable("setting", "max_devices", "0", "VARCHAR(10)");
addFieldToTable("setting", "device_warn_status", "off", "VARCHAR(10)");

// ── ۵) کمپین بازگشت مشتری (Win-back) ─────────────────────────────────
addFieldToTable("setting", "winback_status", "off", "VARCHAR(10)");
addFieldToTable("setting", "winback_days", "30", "VARCHAR(10)");
addFieldToTable("setting", "winback_discount", "20", "VARCHAR(10)");

// ── ۶) مانیتور سلامت پنل ─────────────────────────────────────────────
addFieldToTable("marzban_panel", "last_health_check", "", "VARCHAR(50)");
addFieldToTable("marzban_panel", "health_status", "unknown", "VARCHAR(20)");

// ── ۷) پست خودکار کانال ──────────────────────────────────────────────
addFieldToTable("setting", "channel_autopost_status", "off", "VARCHAR(10)");

// ── تنظیمات کلی کمپین‌ها ─────────────────────────────────────────────
addFieldToTable("setting", "upsell_status", "off", "VARCHAR(10)");
