...
function telegram($method, $datas = [], $token = null)
{
    ...
    $token = trim((string)($token === null ? $APIKEY : $token));
    $method = trim((string)$method);
    if (!preg_match('/^\d{6,20}:[A-Za-z0-9_-]{20,}$/', $token)
        || !preg_match('/^[A-Za-z][A-Za-z0-9_]{1,63}$/', $method)) {
        error_log('Rejected invalid Telegram client parameters.');
        return ['ok' => false, 'description' => 'Invalid Telegram request parameters.'];
    }
    $apiBase = rtrim((string)(rx_env('REDFOX_TELEGRAM_API_BASE', 'https://api.telegram.org') ?: 'https://api.telegram.org'), '/');
    ...
    while ($attemptedTimes < $maxAttempts) {
        ...
        if (function_exists('redfox_apply_curl_proxy')) {
            redfox_apply_curl_proxy($ch, 'telegram');
        }
        ...
        if (empty($policy['ok'])) {
            curl_close($ch);
            return ['ok' => false, 'description' => 'Telegram endpoint is not permitted.'];
        }
    ...
    if ($rawResponse === false) {
        error_log(sprintf('Telegram request failed (method: %s, errno: %d, attempts: %d)',
            preg_replace('/[^A-Za-z0-9_]/', '_', (string)$method), $curlErrorNumber, $attemptedTimes));

        return [
            'ok' => false,
            'description' => 'اتصال به تلگرام در مهلت مقرر برقرار نشد؛ فایروال یا پراکسی خروجی را بررسی کنید.'
        ];
    }
    ...
    if (!is_array($decodedResponse)) {
        error_log(sprintf('Invalid response from Telegram API (method: %s, HTTP %d, bytes: %d)',
            preg_replace('/[^A-Za-z0-9_]/', '_', (string)$method), $httpCode, strlen((string)$rawResponse)));
    ...
    if (isset($decodedResponse['ok']) && !$decodedResponse['ok']) {
        ...
        if (!$rxIsBenign) {
            $rxErrorCode = (int)($decodedResponse['error_code'] ?? 0);
            $rxTgKey = 'tg|' . $rxErrorCode . '|' . hash('sha256', $rxDesc);
            $rxTgLog = '[telegram-api] request failed method=' . preg_replace('/[^A-Za-z0-9_]/', '_', (string)$method) . ' error_code=' . $rxErrorCode;
            if (function_exists('redfox_dedup_error_log')) {
                redfox_dedup_error_log($rxTgKey, $rxTgLog, 21600);
            } else {
                error_log($rxTgLog);
            }
        }
...
{
    $GLOBALS['redfox_update_claim_unavailable'] = false;
    if ((!is_int($updateId) && !is_string($updateId))
        || !preg_match('/^[1-9][0-9]{0,19}$/', (string)$updateId)) {
        return true;
    }

    $pdo = $GLOBALS['pdo'] ?? null;
    if (!($pdo instanceof PDO)) {
        $GLOBALS['redfox_update_claim_unavailable'] = true;
        return true;
    }
    ...
    try {
        rx_require_schema($pdo, ['processed_updates'], [
            'processed_updates' => [
                'status', 'claim_token', 'started_at', 'completed_at',
                'failure_code', 'attempts',
            ],
        ]);

        $claim = $pdo->prepare(
            "INSERT IGNORE INTO processed_updates "
            . "(update_id,processed_at,status,claim_token,started_at,completed_at,failure_code,attempts) "
            . "VALUES(?,?,'processing',?,?,NULL,NULL,1)"
        );
        $claim->execute([(string)$updateId, $now, $claimToken, $now]);
        if ($claim->rowCount() !== 1) {
            return true;
        }

        // Cleanup is deliberately sparse and bounded; rows in processing or
        // failed state are retained for operator review and never auto-retried.
        try {
            if (random_int(1, 100) === 1) {
                $cleanup = $pdo->prepare(
                    "DELETE FROM processed_updates "
                    . "WHERE status='completed' AND processed_at<? ORDER BY processed_at ASC LIMIT 500"
                );
                $cleanup->execute([$now - 604800]);
            }
        } catch (Throwable $ignored) {
        }

        register_shutdown_function(static function () use ($pdo, $updateId, $claimToken): void {
            ...
            $failed = $isFatal || $httpStatus >= 500;
            $failureCode = null;
            if ($isFatal) {
                $failureCode = 'php_fatal_type_' . (int)($fatal['type'] ?? 0);
            } elseif ($httpStatus >= 500) {
                $failureCode = 'http_' . $httpStatus;
            }
            try {
                $finish = $pdo->prepare(
                    "UPDATE processed_updates SET status=?,completed_at=?,failure_code=? "
                    . "WHERE update_id=? AND claim_token=? AND status='processing'"
                );
                $finish->execute([
                ...
                error_log('Telegram update finalization failed: ' . redfox_exception_fingerprint($finishError));
            }
        });
        return false;
    } catch (Throwable $e) {
        $GLOBALS['redfox_update_claim_unavailable'] = true;
        error_log('Telegram update claim failed: ' . redfox_exception_fingerprint($e));
        return true;
    }
}
...
if (!defined('REDFOX_ALLOW_BOTAPI_ROUTER') || REDFOX_ALLOW_BOTAPI_ROUTER !== true) {
    return;
}
...
if (defined('REDFOX_SKIP_BOTAPI_ROUTER') && REDFOX_SKIP_BOTAPI_ROUTER) {
    return;
}


$rx_raw_body = @file_get_contents('php://input', false, null, 0, 1048577);
if (!is_string($rx_raw_body) || $rx_raw_body === '' || strlen($rx_raw_body) > 1048576) {
    return;
}


$rx_update_probe = json_decode($rx_raw_body, true, 32);
if (!is_array($rx_update_probe) || json_last_error() !== JSON_ERROR_NONE) {
    return;
}
...
$rxIncomingSecret = isset($_SERVER['HTTP_X_TELEGRAM_BOT_API_SECRET_TOKEN'])
    ...
    : '';
if (strlen($rxStoredSecret) < 32 || $rxIncomingSecret === '' || !hash_equals($rxStoredSecret, $rxIncomingSecret)) {
        $rxRejectIp = (string) redfox_client_ip();
...
if (isDuplicateUpdate($update_id)) {
    if (!headers_sent()) {
        http_response_code(!empty($GLOBALS['redfox_update_claim_unavailable']) ? 503 : 200);
    }
    exit;
}
