...
function panel_login_cookie($code_panel)
{
    ...
    redfox_apply_panel_curl_url_policy($curl, (string)$panel['url_panel']);
...
function get_clinets($username, $namepanel)
{
    ...
    if (!empty($response['error']) && stripos((string) $response['error'], 'Inbound Not Found For Email') === false) {
        $dedupKey = 'xui_single_resp|' . hash('sha256', (string)$response['error']);
        $safeLog = '[x-ui-single] panel request failed HTTP=' . (int)($response['status'] ?? 0);
        if (function_exists('redfox_dedup_error_log')) {
            redfox_dedup_error_log($dedupKey, $safeLog);
        } else {
            error_log($safeLog);
        }
    }
