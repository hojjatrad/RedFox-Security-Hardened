...
function guardDecodeResponse(array $response)
{
    if (!empty($response['error'])) {
        return [
            'status' => false,
            'msg' => redfox_remote_error_summary($response),
            'error_code' => redfox_remote_error_summary($response)
        ];
    }
    ...
    if ($statusCode !== null && $statusCode >= 400) {
        $safeFailure = is_array($decodedBody) ? $decodedBody : [];
        $safeFailure['http_status'] = (int)$statusCode;
        return [
            'status' => false,
            'msg' => redfox_remote_error_summary($safeFailure),
            'error_code' => redfox_remote_error_summary($safeFailure)
        ];
    }
...
function guardTestConnection($baseUrl, $apiKey)
{
    ...
    if ($decoded['status'] === false) {
        $safeCode = (string)($decoded['error_code'] ?? $decoded['msg'] ?? 'remote_failure');
        return [
            'status' => false,
            'msg' => $safeCode,
            'error_code' => $safeCode
