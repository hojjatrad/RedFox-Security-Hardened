...
function token_panelm($code_panel){
    if (isset($GLOBALS['__redfox_panel_token_cache']['marzneshin'][$code_panel])) {
        $cached = $GLOBALS['__redfox_panel_token_cache']['marzneshin'][$code_panel];
    ...
    if($panel['datelogin'] != null){
        ...
        if($start_date <= 3600){
            $GLOBALS['__redfox_panel_token_cache']['marzneshin'][$code_panel] = [
                '__cached_at' => time(),
                'payload'     => $date,
            ];
            return $date;
    ...
    if (function_exists('redfox_apply_curl_proxy')) redfox_apply_curl_proxy($curl_token, 'panel');
    curl_setopt_array($curl_token, $options);
    redfox_apply_panel_curl_url_policy($curl_token, $url_get_token);
