...
# ─── PATHS ─────────────────────────────────────────────────────────────────
readonly BOT_DIR="/var/www/html/redfox"
readonly CRED_DIR="/root/confredfox"
readonly CRED_FILE="${CRED_DIR}/dbrootredfox.txt"
readonly LOG_FILE="/var/log/redfox_installer.log"
readonly TMP_DOWNLOAD="/tmp/redfox_download"
readonly TMP_UPDATE="/tmp/redfox_update"
readonly DEFAULT_DB_NAME="redfox"
readonly INSTALL_SCRIPT_PATH="/root/install.sh"
readonly INSTALL_SCRIPT_LINK="/usr/local/bin/redfox"
...
increase_upload_limit() {
    ...
    for ini in "${INIS[@]}"; do
        ...
        redfox_set_ini "max_execution_time"  "600"         "$ini"
        redfox_set_ini "max_input_time"      "600"         "$ini"
        ui_ok "Updated ${ini}"
        ((changed++))
    done
...
install_bot() {
    ...
    if [ -z "${REDFOX_PHPMYADMIN_PASS}" ]; then
        REDFOX_PHPMYADMIN_PASS="$(head -c 24 /dev/urandom | base64 | tr -d '/+=' | cut -c1-28)"
    fi
    echo 'phpmyadmin phpmyadmin/dbconfig-install boolean true' | debconf-set-selections
    printf "phpmyadmin phpmyadmin/app-password-confirm password %s\n" "${REDFOX_PHPMYADMIN_PASS}" | debconf-set-selections
    ...
    if [[ "$1" == "-v" && "$2" == "beta" ]] || [[ "$1" == "-beta" ]] || [[ "$1" == "-" && "$2" == "beta" ]]; then
        ZIP_URL="${REDFOX_GITHUB}/archive/refs/heads/main.zip"
    elif [[ "$1" == "-v" && -n "$2" ]]; then
        ZIP_URL="${REDFOX_GITHUB}/archive/refs/tags/$2.zip"
    fi
    if [ -z "$ZIP_URL" ]; then
    ...
    if [ ! -d "$CRED_DIR" ]; then
        ...
        local ASAS='$'
        {
            ...
            echo "${ASAS}path = '${RANDOM_NUMBER:-red fox}';"
        } > "$CRED_FILE"
...
${ASAS}dbname = '${dbname}';
${ASAS}domainhosts = '${YOUR_DOMAIN}/redfox';
...
EOF
    ...
    curl -F "url=https://${YOUR_DOMAIN}/redfox/index.php" \
    ...
    ui_action "Initialising database tables via PHP CLI..."
    php "$BOT_DIR/table.php" >/dev/null || {
        ui_warn "Schema setup failed; run php $BOT_DIR/table.php manually."
    }
...
EOF
    ...
    ui_action "Setting up database tables via PHP CLI..."
    php "$BOT_DIR/table.php" >/dev/null || {
        ui_warn "Schema setup failed; run php $BOT_DIR/table.php manually."
    }
...
update_bot() {
    ...
    if [[ "$1" == "-beta" ]] || [[ "$1" == "-v" && "$2" == "beta" ]]; then
        ...
        ZIP_URL=$(curl -s "https://api.github.com/repos/${REDFOX_REPO}/releases/latest" | grep "zipball_url" | cut -d '"' -f4)
        [ -z "$ZIP_URL" ] && ZIP_URL="${REDFOX_GITHUB}/archive/refs/heads/main.zip"
    fi
    ...
    local TEMP_CONFIG="/root/redfox_config_backup.php"
    if [ -f "$CONFIG_PATH" ]; then
    ...
    if [ -n "$local_install" ]; then
        ...
        local raw_url="https://raw.githubusercontent.com/${REDFOX_REPO}/main/install.sh"
    ...
    if [ -n "$URL" ]; then
        ...
        CLEAN_URL=${CLEAN_URL%/}
        php "$BOT_DIR/table.php" >/dev/null || {
            ui_warn "Schema setup failed for $BOT_DIR/table.php"
        }
    else
...
remove_bot() {
    show_logo
    ui_panel "REMOVE REDFOX BOT" "$C_BOLD$C_RED" "$C_RED" \
...
auto_backup() {
    ...
    if check_marzban_installed; then
        ...
        BACKUP_SCRIPT="/root/backup_redfox_marzban.sh"
        MYSQL_CONTAINER=$(docker ps -q --filter "name=mysql" --no-trunc)
        if [ -z "$MYSQL_CONTAINER" ]; then
...
EOF
        ...
        BACKUP_SCRIPT="/root/redfox_backup.sh"
        if ! mysql -u "$DB_USER" -p"$DB_PASS" -e "USE ${DB_NAME};" 2>/dev/null; then
            ui_err "Database ${DB_NAME} does not exist or credentials are incorrect."
            return 1
        fi
...
#  (i.e. *separate* from the main /var/www/html/redfox bot). They re-use the
...
EOF
    ...
    local DB_NAME="redfox_${BOT_NAME}"
...
EOF
    ...
    ui_action "Setting up database tables via PHP CLI..."
    php "$BOT_PATH/table.php" >/dev/null || \
        ui_warn "Schema setup failed; run php $BOT_PATH/table.php manually."
...
update_additional_bot() {
    ...
    if ! git clone "${REDFOX_GITHUB}.git" "$BOT_PATH"; then
        ui_err "Failed to clone the repository."
        return 1
    fi
    ...
    if ! php "$BOT_PATH/table.php" >/dev/null; then
        ui_warn "Failed to execute schema setup; run php $BOT_PATH/table.php manually."
    fi
