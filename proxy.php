...
/**
 * Outbound proxy support for Red Fox.
 *
...
if (!function_exists('redfox_proxy_settings')) {
    function redfox_proxy_settings($forceReload = false)
    {
        static $cache = null;
...
if (!function_exists('redfox_parse_proxy_url')) {
    /**
     ...
     */
    function redfox_parse_proxy_url($url)
    {
        $url = trim((string) $url);
        if ($url === '') {
            return null;
...
if (!function_exists('redfox_proxy_for')) {
    /**
     ...
     */
    function redfox_proxy_for($scope)
    {
        $s = redfox_proxy_settings();
        if ($scope === 'telegram') {
        ...
        return redfox_parse_proxy_url($url);
    }
}
...
if (!function_exists('redfox_apply_curl_proxy')) {
     ...
     */
    function redfox_apply_curl_proxy($ch, $scope)
    {
        if (!is_resource($ch) && !($ch instanceof \CurlHandle)) {
            return false;
        }
        $proxy = redfox_proxy_for($scope);
        if ($proxy === null) {
            return false;
        }
