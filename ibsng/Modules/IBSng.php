...
class IBSng
{
    ...
    public function connect()
    {
        ...
        try {
            ...
            throw new \RuntimeException('IBSng request failed', 0, $ex);
        }
    ...
    protected function infoByUsername($username, $withPassword = false, $output = null)
    {
        ...
        $group_name = substr($group_trim1, 0, $group_pos2);
    ...
    protected function request($url, $postData = array(), $header = FALSE)
    {
        ...
        if (function_exists('redfox_apply_curl_proxy')) redfox_apply_curl_proxy($this->handler, 'panel');
        ...
        redfox_apply_panel_curl_url_policy($this->handler, $url);
            $output = curl_exec($this->handler);
        if (curl_errno($this->handler) != 0) {
            throw new \RuntimeException('IBSng transport error ' . (int)curl_errno($this->handler));
        }
        curl_close($this->handler);
        return $output;
    }
