...
function token_panel($code_panel,$verify = true){
    if ($verify && isset($GLOBALS['__redfox_panel_token_cache']['marzban'][$code_panel])) {
        $cached = $GLOBALS['__redfox_panel_token_cache']['marzban'][$code_panel];
    ...
    if($panel['datelogin'] != null && $verify){
        ...
        if($start_date <= 3600){
            $GLOBALS['__redfox_panel_token_cache']['marzban'][$code_panel] = [
                '__cached_at' => time(),
                'payload'     => $date,
            ];
            return $date;
